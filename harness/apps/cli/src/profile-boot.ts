...
async function composeProfile(
  ...
  // Heal the shared module fallback BEFORE resolving bundle names: a profile
  // template can list bundles that ride the installation closure but no
  // profile-local install (the shipped web plugins), and their names resolve
  // through $DSH_HOME/profiles/node_modules, which only this heal links. The
  // profile-scoped pass (bundle-only packages kept profile-local) still runs
  // after the load, exactly as before.
