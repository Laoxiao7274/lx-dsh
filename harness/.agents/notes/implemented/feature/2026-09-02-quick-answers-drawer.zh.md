...
- **快问交流是主窗口内的左侧锚定收缩抽屉**——面板（420px、l2 浮层）右缘带边缘把手，承载针对 `quick-answers` 预设（搜索先行、附来源、即问即弃）的紧凑问答交流。抽屉壳（面板 + 把手）两种状态都保持挂载：点把手切换收缩，收起时面板滑出屏幕、把手停靠左缘，滚动与草稿状态在收缩后保留（面板体经 `visibility` 移出无障碍树）。没有单独的 ✕ 控制——把手与侧栏底部 ⚡ 入口即切换。纯客户端插件工作：完全不需要 Electron 侧代码。
...
**右锚定滑入 + ✕（抽屉第一版）。** 关闭按钮读作「关闭」且每次关闭都丢弃面板状态。否决、改为收缩把手：左锚定面板滑出屏幕、把手停靠，滚动与草稿状态保活，一个控件覆盖两个方向（✕ 已移除）。

**收起时单独渲染左缘把手。** 两个元素（面板/把手）交叉淡入淡出。否决：一个常驻挂载的壳体 + transform 更简单，DOM 稳定，一条规则双向动画。

## 后果

一击即在主对话旁打开有据可查的问答交流；其会话是普通的持久会话（reset 时归档），主窗口的选择从不受影响。收起态把手停靠在侧栏左缘之上（抽屉按设计占据壳的侧栏一侧——展开时同样覆盖侧栏）。已知限制：一个抽屉一个线程——切换工作区不关闭它（快问会话按构造无工作区）；预设的搜索活动只显示运行点点（工具调用进度行留作打磨）；轮次历史只随事件窗口的尾页（抽屉从不分页更早的轮次）。验证：ui-lx-shell 规范（40 测试——按钮、抽屉交互含收缩把手契约、轮次投影、apply 接线）、test:gui 绿、client + host tsc 0，以及 dev 实例的 CDP 端到端（点击 → 抽屉 → 两个真实问题、流式带链接渲染的回答）。
