...
export interface ConnectionTrustRequest {
  ...
  /**
   * Request URL when the carrier exposes it (node:http always does; the
   * upgrade path reads the `token` query parameter for browser WebSocket
   * clients, which cannot set an Authorization header).
   */
