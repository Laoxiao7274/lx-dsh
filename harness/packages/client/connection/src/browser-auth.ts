...
/** Read the launch token from an `Authorization: Bearer <token>` header. */
function bearerToken(headers: ConnectionTrustRequest['headers']): string | undefined {
  const value = header(headers, 'authorization')
  if (value === undefined) return undefined
  const scheme = value.slice(0, 7)
...
export class BrowserAuth {
  ...
  isAuthenticated(request: ConnectionTrustRequest): boolean {
    ...
    if (bearer !== undefined) return this.verifyLaunchToken(bearer)
  ...
  /**
   * Verify a presented value against this process's launch token. This is
   * the non-browser credential: an API client passes it as `Authorization:
   * Bearer <token>` on HTTP requests, or as the `token` query parameter on a
   * WebSocket upgrade (browsers cannot set upgrade headers).
   * @param value - untrusted presented token.
   * @returns true only on an exact constant-time match.
   */
  verifyLaunchToken(value: string): boolean {
    return tokenMatches(value, this.launchToken)
  }
