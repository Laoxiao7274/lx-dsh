 ...
 *
 * Each row declares its request modalities through a vision toggle: on writes
 * `inputModalities: ['text', 'image']`, off clears the field back to the
 * adapter's text-only default. Rows enter with a short gsap fade/slide (the
 * newly added one alone on append), a disclosure slides open, and the vision
 * chip pops on toggle — all gated by the shared motion-allowed reading.
 */
...
import {
  IconChevronDownOutline14, IconChevronRightOutline14, IconPlusOutline16, IconTrashOutline16, motionAllowed,
} from '@deepseek-ai/dsh-client-ui-primitives'
import { ModelVisionChip, ModelVisionSwitch } from './VisionChip.tsx'
...
/** Whether a draft row declares image input (the vision toggle's mirror). */
function visionOn(model: DeepSeekModelDraft): boolean {
...
/**
 * Render the direct DeepSeek adapter's model catalog: id, display name, and
 * the vision toggle on each row, capacities behind the row's own disclosure.
 * Rows enter with a short gsap slide-fade (staggered on first paint, single
 * row on append), a disclosure slides open, and the vision chip pops — every
 * effect gated by the shared motion-allowed reading.
...
export function DeepSeekModelsEditor(props: DeepSeekModelsEditorProps): ReactNode {
  ...
  const prevCount = useRef(props.models.length)
  const panelRefs = useRef(new Map<number, HTMLDivElement>())
  const prevExpanded = useRef<ReadonlySet<number>>(expanded)

  // First paint staggers the rows; an append slides the new row in alone.
  // Rows below a removal re-key (fresh mounts) and re-enter, which reads as
  // the list settling — the intended reflow. Each run owns a gsap context so
  // a superseded or unmounting tween reverts its inline styles cleanly.
  useEffect(() => {
    const list = listRef.current
    const grew = props.models.length > prevCount.current
    const firstPaint = prevCount.current === 0
    prevCount.current = props.models.length
    if (list === null || !motionAllowed() || props.models.length === 0) return undefined
    const ctx = gsap.context(() => {
      if (firstPaint) {
        gsap.from(Array.from(list.children), { opacity: 0, y: -6, duration: 0.24, stagger: 0.05, ease: 'power2.out' })
      } else if (grew) {
        const added = list.children[props.models.length - 1]
        if (added !== undefined) gsap.from(added, { opacity: 0, y: -6, duration: 0.24, ease: 'power2.out' })
      }
    }, list)
    return () => { ctx.revert() }
  }, [props.models.length])

  // A newly expanded disclosure slides open; the panel mounts fresh, so its
  // gsap.from runs once per expansion. The context reverts a tween whose panel
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      for (const at of expanded) {
        if (prevExpanded.current.has(at)) continue
        const panel = panelRefs.current.get(at)
        if (panel !== undefined) gsap.from(panel, { height: 0, opacity: 0, duration: 0.22, ease: 'power2.out' })
      }
    })
    prevExpanded.current = expanded
    return () => { ctx.revert() }
  ...
  /**
   * Toggle the row's image input modality. On adds `image` beside the
   * existing entries; off removes it, falling back to the adapter's text-only
   * default by clearing the field when nothing else remains.
   */
  const toggleVision = (index: number): void => {
    const model = props.models[index]
    if (model === undefined) return
    const current = Array.isArray(model['inputModalities'])
      ? (model['inputModalities'] as unknown[]).filter((m): m is string => typeof m === 'string')
      : ['text']
    const next = current.includes('image')
      ? current.filter(modality => modality !== 'image')
      : Array.from(new Set(['text', ...current, 'image']))
    const textOnly = next.length === 1 && next[0] === 'text'
    update(index, 'inputModalities', next.length === 0 || textOnly ? undefined : next)
  }
            ...
            {props.models.map((model, index) => (
              <div className={styles['modelEntry']} key={index}>
                <div className={styles['modelRow']}>
                  ...
                  <ModelVisionChip
                    on={visionOn(model)}
                    label={`${props.t('modelVision')} ${String(index + 1)}`}
                    disabled={props.disabled}
                    onToggle={() => { toggleVision(index) }}
                  />
                  <button
                    type="button"
                ...
                {expanded.has(index)
                  ? (
                    <div
                      className={styles['modelAdvanced']}
                      ref={(el) => {
                        if (el === null) panelRefs.current.delete(index)
                        else panelRefs.current.set(index, el)
                      }}
                    >
                      ...
                      <div className={styles['modelVisionField']}>
