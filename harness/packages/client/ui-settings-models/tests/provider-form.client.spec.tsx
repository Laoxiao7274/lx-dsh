...
/** Open the editor of one configured row; its fields and models render flat. */
...
describe('model list editing', () => {
  ...
  it('declares image input per row through the vision chip', async () => {
    const { mutate } = await mountSection()
    openEditor('openai')

    fireEvent.click(screen.getByRole('button', { name: en.addModel }))
    fireEvent.change(screen.getByLabelText(`${en.modelId} 1`), { target: { value: 'acme-vision' } })
    // The chip starts unpressed (no explicit input list); toggling writes the
    // explicit text+image list a hand-declared vision model needs.
    expect(screen.getByLabelText(`${en.modelVision} 1`).getAttribute('aria-pressed')).toBe('false')
    fireEvent.click(screen.getByLabelText(`${en.modelVision} 1`))
    expect(screen.getByLabelText(`${en.modelVision} 1`).getAttribute('aria-pressed')).toBe('true')
    fireEvent.click(screen.getByText(en.apply))

    await waitFor(() => { expect(mutate).toHaveBeenCalled() })
    expect(firstMutate(mutate)).toMatchObject({
      ns: 'llm-pi-ai',
      ops: [{ op: 'set', path: ['providers', 'openai', 'models'], value: [{ id: 'acme-vision', input: ['text', 'image'] }] }],
    })
  })
