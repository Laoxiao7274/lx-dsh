...
/**
 * Built-in skins ship inside the ui-theme package under skins/. Resolution
 * walks upward from this module until it finds the package root (the
 * directory holding package.json): the tree differs between source runs
 * (src/skin-center/), the built node bundle (lib/index.js), and the
 * deployed profile layout, so probing beats hard-coding one relative depth.
 */
export function builtinSkinsDir(fromUrl: string = import.meta.url): string {
  ...
  for (let i = 0; i < 6; i += 1) {
    ...
    if (existsSync(manifest)) {
      try {
        ...
        if (pkg.name === '@deepseek-ai/dsh-client-ui-theme') {
          return existsSync(candidate) ? candidate : manifest // missing dir → package root (empty catalog)
        }
      } catch {
  ...
  // Fall back to the source layout depth (src/skin-center -> package root).
  return join(dirname(fileURLToPath(fromUrl)), '..', '..')
}

/**
 * Shipped builtin skin ids: the package.json files whitelist entries under
 * `skins/` (the "<id>/" directory name).
 */
  ...
  try {
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      if (existsSync(pkgPath)) {
        try {
          const pkg = JSON.parse(readFileSync(pkgPath, 'utf8')) as { name?: string; files?: unknown }
          if (pkg.name === '@deepseek-ai/dsh-client-ui-theme') {
...
export async function repairSkin(
  ...
  const res = await repairSkinFromMarket(destDir, skinId, {
    ...
    ...(options.localSourceDir === undefined ? {} : { localSourceDir: options.localSourceDir }),
  })
  if (res.ok) {
    const cache = options.catalogCache ?? DEFAULT_CATALOG_CACHE
    cache.clear()
