...
/** Persisted background display row shape (mirrors the theme snapshot). */
export interface BackgroundRowStateBackground {
  fileName: string
  mediaType: string
  opacity: number
}

/** Store state mirrored from the theme snapshot's background row. */
export interface BackgroundRowState {
  /** The durable background display row; null while no image is stored. */
  background: BackgroundRowStateBackground | null
  /** Service revision; -1 until first sync so revision 0 lands as a change. */
  revision: number
}
...
type BackgroundRowActions = {
  sync: (draft: BackgroundRowState, background: BackgroundRowStateBackground | null, revision: number) => void
}
...
export function createBackgroundRowStore(): EngineStoreHandle<BackgroundRowState, BackgroundRowActions> {
  return defineStore({
    init: (): BackgroundRowState => ({ background: null, revision: -1 }),
    actions: {
      sync: (d, background: BackgroundRowStateBackground | null, revision: number) => {
        if (revision <= d.revision) return
