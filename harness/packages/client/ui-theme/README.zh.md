...
`dsh-client-ui-theme` 让 Web GUI 用户在设置中选择 `light`、`dark` 或 `system`，把会话正文字号设为 12 至 17 px，经皮肤中心切换皮肤，并设置背景图。回环客户端把这些值存入 `ui-theme` 设置命名空间，本地提供方持久化到 `$DSH_HOME/settings.yaml`（背景图文件在 `$DSH_HOME/appearance/background.img`；皮肤在包内 `skins/` 与 `$DSH_HOME/skins/`）。插件通过 `prefers-color-scheme` 解析 `system` 并发布不可变的 `ThemeSnapshot`；ui-layout 把每份快照应用到 document，背景投射器在应用外壳之下挂载固定图片层。本包还提供 `--dsw-*` token 样式表并注入同步引导，使所选调色板与字号在外壳加载前生效（活动皮肤走 v2 通道——index tap 预应用其样式表）。第三方主题可通过 `ctx.theme` 注册别名 token 覆盖。
...
用户在专属的「外观」设置分区中切换配色方案、皮肤与背景图；字号行保留在「通用」分区（它影响会话正文，不是界面外观）。在回环浏览器上，所有选择都会跨重启持久化。功能插件通过 `ctx.theme` 消费当前快照，并在 CSS 中读取 `--dsw-*` token；它们不自行管理主题状态。

### 外观分区

插件注册自己的设置导航入口（位于模型与插件之间），分区通过 `settings.appearance.item` 列表槽堆叠三个功能行：方案方块、皮肤卡片与背景图行。正文字号步进器留在通用分区。

### 外观、皮肤与背景

方案方块切换 `light`/`dark`/`system`；皮肤卡在内置皮肤间切换；背景行上传、调节或清除背景图。

皮肤是一张叠加在两个基础调色板之上的 token 覆盖表：它从不绑定配色方案，外观偏好始终是方案权威，亮暗切换会把同一套皮肤重组为另一调色板的取值。皮肤层叠在 `overrideTokens` 覆盖层之下（包级覆盖按 token 胜过皮肤）。存储中的未知 id 在采纳时回落到 `default`。

背景图是单个持久文件（`$DSH_HOME/appearance/background.img`，≤ 8 MiB，png/jpeg/webp/gif/avif），经回环宿主路由维护（`/api/ui-theme/background` 提供图片；`/upload` 与 `/clear` 与设置行——fileName、mediaType、opacity 0.1–1——同进退）。客户端把它投射为应用外壳之下的全屏固定层（`pointer-events: none`），随每份主题快照重新挂载；引导脚本在首帧前挂同一层，因此刷新不闪。非回环浏览器隐藏该行。
...
当主机组合包含 HTTP 服务器时，宿主侧会把已注册的 `ui-theme` 设置或 schema 默认值嵌入每份 index 响应。浏览器在加载页面渲染前设置 `color-scheme`、`body[data-ds-dark-theme]`、`--dsh-content-font-size`、活动皮肤的 token 变量与背景图层，因此首帧绘制就采用所选调色板与字号，不闪回无皮肤无壁纸的默认值。
