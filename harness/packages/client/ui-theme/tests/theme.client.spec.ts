...
describe('ThemeRuntime', () => {
  ...
  it('setSkin is a pass-through marker write (the v2 channel owns the real switch)', () => {
    const { theme, events, host } = make()
    theme.setSkin('blue-fantasy')
    expect(theme.getTheme().skin).toBe('blue-fantasy')
    expect(host.set).toHaveBeenCalledWith('skin', 'blue-fantasy')
    expect(events).toHaveLength(1)
    // No token composition — skin CSS is injected by the skin-center runtime.
    expect(theme.getTheme().active.tokens['--dsw-alias-brand-primary']).toBeUndefined()
    // A scheme flip does not touch the skin marker.
    theme.setTheme('dark')
    expect(theme.getTheme().skin).toBe('blue-fantasy')
  ...
  it('setSkin back to default clears the marker without touching tokens', () => {
    const { theme } = make()
    theme.setSkin('verdigris')
    theme.setSkin('default')
    expect(theme.getTheme().skin).toBe('default')
    expect(theme.getTheme().active.tokens['--dsw-alias-brand-primary']).toBeUndefined()
  })

  it('adopts a published Host skin marker verbatim (catalog validation lives in the v2 channel)', () => {
    const { theme, events, host } = make()
    host.publish({ status: 'ready', value: { preference: 'system', fontSize: 14, skin: 'midnight-ember' }, revision: 1, writable: true })
    expect(theme.getTheme().skin).toBe('midnight-ember')
    expect(events).toHaveLength(1)
    expect(host.set).not.toHaveBeenCalled()
  })
