 ...
 * projection into the row stores, and HMR collapse recovery. The skin-center
 * row rides the v2 runtime (node lanes have no document, so the row never
 * registers there — covered by the skin-center lane instead). */
...
import type {
  AppearanceRowInjected, BackgroundRowInjected, FontSizeRowInjected, ThemeRuntime,
...
const APPEARANCE_SLOT = 'settings.appearance.item'
...
async function bench(isLoopback = true) {
  ...
  const section: Record<string, unknown> = { preference: 'system', fontSize: 14, skin: 'default', background: null }
  const namespace = () => ({
...
/** Stand in for the settings shell: declare the section and General item slots
 * from root. The appearance item slot is declared at runtime by the
 * AppearanceSection entry, exactly as production composes it. */
...
function faceOf(slots: SlotRegistry) {
  const entry = slots.entries(APPEARANCE_SLOT).find(e => e.component === AppearanceRow)!
...
/** The same choreography for the background row entry. */
function backgroundFaceOf(slots: SlotRegistry) {
  const entry = slots.entries(APPEARANCE_SLOT).find(e => e.component === BackgroundRow)!
...
describe('ui-theme apply', () => {
  ...
  it('provides the service, registers localized copy, and registers the Appearance section (declaration before or after apply)', async () => {
    ...
    // The Appearance section entry itself rides the section slot.
    const sectionEntry = before.slots.entries(SECTION_SLOT).find(e => e.component === AppearanceSection)!
    expect(sectionEntry.options).toMatchObject({ id: 'appearance', order: 12 })
    // Its rows ride the appearance item slot. The skin-center row rides the
    // v2 runtime and needs a document — this node lane registers scheme and
    // background only (the skin row's coverage is the skin-center lane).
    const schemeEntry = before.slots.entries(APPEARANCE_SLOT).find(e => e.component === AppearanceRow)!
    expect(schemeEntry.options).toMatchObject({ id: 'scheme', order: 0 })
    expect(before.locale.bind(SETTINGS_NS)('skin.title')).toBe('皮肤')
    const backgroundEntry = before.slots.entries(APPEARANCE_SLOT).find(e => e.component === BackgroundRow)!
    expect(backgroundEntry.options).toMatchObject({ id: 'background', order: 20 })
    // The font-size row stays in the General section.
    ...
    expect(before.locale.bind(SETTINGS_NS)('skin.title')).toBe('Skin')

    const after = await bench()
    const fiber = after.ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    expect(after.slots.entries(SLOT)).toHaveLength(0)
    expect(after.slots.entries(APPEARANCE_SLOT)).toHaveLength(0)
    ...
    expect(after.slots.entries(APPEARANCE_SLOT).some(e => e.component === AppearanceRow)).toBe(true)
    expect(after.slots.entries(APPEARANCE_SLOT).some(e => e.component === BackgroundRow)).toBe(true)
  })
  ...
  it('projects service snapshots into the row store and routes face writes back', async () => {
    ...
    expect(b.slots.entries(APPEARANCE_SLOT).find(e => e.component === AppearanceRow)!.locale).toBe(SETTINGS_NS)
  ...
  it('projects skin snapshots into the theme marker and routes pass-through writes back', async () => {
    const b = await bench()
    declareItems(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const theme = b.ctx.get('theme') as ThemeRuntime
    // The skin field is a pass-through marker (the v2 channel owns the real
    // switch); unknown ids carry through without registry validation.
    theme.setSkin('blue-fantasy')
    expect(theme.getTheme().skin).toBe('blue-fantasy')
    theme.setSkin('default')
    expect(theme.getTheme().skin).toBe('default')
  ...
  it('recovers after an HMR collapse of the declaring entry (stale disposer must not block)', async () => {
    ...
    expect(b.slots.entries(APPEARANCE_SLOT)).toHaveLength(2)
    ...
    expect(b.slots.entries(APPEARANCE_SLOT)).toHaveLength(0)

    declareItems(b.slots)
    await Promise.resolve()
    expect(b.slots.entries(SLOT).some(e => e.component === FontSizeRow)).toBe(true)
    expect(b.slots.entries(APPEARANCE_SLOT).some(e => e.component === AppearanceRow)).toBe(true)
    expect(b.slots.entries(APPEARANCE_SLOT).some(e => e.component === BackgroundRow)).toBe(true)
  })
  ...
  it('hides the background row on non-loopback browsers and shows it on loopback', async () => {
    const remote = await bench(false)
    declareItems(remote.slots)
    await remote.ctx.plugin({ inject: [...inject], apply }).await()
    expect(remote.slots.entries(APPEARANCE_SLOT).some(e => e.component === BackgroundRow)).toBe(false)
    await remote.ctx.fiber.dispose()

    const local = await bench(true)
    declareItems(local.slots)
    await local.ctx.plugin({ inject: [...inject], apply }).await()
    expect(local.slots.entries(APPEARANCE_SLOT).some(e => e.component === BackgroundRow)).toBe(true)
    await local.ctx.fiber.dispose()
  })

  it('projects background snapshots into its row store and routes opacity writes back', async () => {
    const b = await bench()
    declareItems(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const theme = b.ctx.get('theme') as ThemeRuntime
    theme.setBackground({ fileName: 'background.img', mediaType: 'image/png', opacity: 0.6 })

    const { instance, face } = backgroundFaceOf(b.slots)
    expect(instance.getSnapshot().background).toMatchObject({ opacity: 0.6 })

    face.setOpacity(0.9)
    expect(theme.getTheme().background?.opacity).toBe(0.9)
    expect(instance.getSnapshot().background?.opacity).toBe(0.9)
    await vi.waitFor(() => { expect(b.mutate).toHaveBeenCalled() })
  })
})
