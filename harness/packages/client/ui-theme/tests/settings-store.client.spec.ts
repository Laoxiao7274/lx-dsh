/** Appearance, font-size, and background row stores: snapshot-mirror actions and the revision guards. */
import { describe, expect, it } from 'vitest'
import { createAppearanceRowStore, createFontSizeRowStore, createBackgroundRowStore } from '../src/client/settings-store.ts'
...
describe('createBackgroundRowStore', () => {
  it('init shape: null background with revision at -1', () => {
    const store = createBackgroundRowStore().create()
    expect(store.getSnapshot()).toEqual({ background: null, revision: -1 })
  })

  it('sync mirrors the background row; the revision guard drops stale and duplicate writes', () => {
    const store = createBackgroundRowStore().create()
    store.actions.sync({ fileName: 'background.img', mediaType: 'image/png', opacity: 0.5 }, 3)
    expect(store.getSnapshot().background).toEqual({ fileName: 'background.img', mediaType: 'image/png', opacity: 0.5 })
    store.actions.sync(null, 2)
    store.actions.sync(null, 3)
    expect(store.getSnapshot().background).toEqual({ fileName: 'background.img', mediaType: 'image/png', opacity: 0.5 })
    expect(store.getSnapshot().revision).toBe(3)
  })
})
