...
`dsh-client-ui-theme` lets Web GUI users choose `light`, `dark`, or `system`, set conversation content text from 12 to 17 px, switch skins through the skin center, and set a background image in Settings. A loopback client stores all values in the `ui-theme` settings namespace, which the local provider persists in `$DSH_HOME/settings.yaml` (the background image itself in `$DSH_HOME/appearance/background.img`; skins live in the package `skins/` and `$DSH_HOME/skins/`). The plugin resolves `system` through `prefers-color-scheme` and publishes immutable `ThemeSnapshot`s; ui-layout applies each snapshot to the document, and the background presenter mounts a fixed image layer under the app shell. The package also ships the `--dsw-*` token stylesheets and injects a synchronous bootstrap so the selected palette and font size apply before the shell loads (the active skin rides the v2 channel — the index tap pre-applies its stylesheet). Third-party themes can register alias-token overrides through `ctx.theme`.
...
Users switch the color scheme, content font size, skin, and background from one dedicated Appearance settings section; the choices persist across restarts on a loopback browser. Feature plugins consume the current snapshot through `ctx.theme` and read the `--dsw-*` tokens in CSS; they do not manage theme state themselves.

### The Appearance section

The plugin registers its own Settings nav entry (between Models and Plugins) whose section stacks three feature-owned rows through the `settings.appearance.item` list slot: the scheme cubes, the skin cards, and the background-image row. The content font-size stepper stays in the General section (it sizes conversation content, not the chrome).

### Appearance, skin, and background

The scheme cubes switch `light`/`dark`/`system`; the font-size stepper accepts 12–17 px; the skin cards switch between the built-in skins; the background row uploads, adjusts, or clears the background image.

The skin is a token-override table layered over both base palettes: it never binds a color scheme, so the Appearance preference stays the scheme authority and a light/dark flip re-composes the same skin for the other palette. The skin layer composes under `overrideTokens` layers (a package override wins per-token over the skin). An unknown stored id falls back to `default` at adoption.

The background image is one durable file (`$DSH_HOME/appearance/background.img`, ≤ 8 MiB, png/jpeg/webp/gif/avif) written through the loopback host routes (`/api/ui-theme/background` serves it; `/upload` and `/clear` maintain it together with its settings row: fileName, mediaType, opacity 0.1–1). The client projects it as a fixed full-viewport layer behind the app shell (`pointer-events: none`), re-mounted from every theme snapshot, and the boot script mounts the same layer before first paint so a reload never flashes. Non-loopback browsers hide the row.
...
When the host composition includes an HTTP server, the host half embeds the registered `ui-theme` settings, or schema defaults, into each index response. Before the loading page renders, the browser sets `color-scheme`, `body[data-ds-dark-theme]`, `--dsh-content-font-size`, the active skin's token variables, and the background image layer, so the first paint uses the selected palette and text size without flashing the un-skinned, un-wallpapered defaults.
