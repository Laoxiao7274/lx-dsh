...
import { TrajectoryToolbar, type TrajectoryTimelineModeChoice } from './TrajectoryToolbar.tsx'
...
export function TrajectoryView({
  ...
  & PropsRenderSlots<'conversation.trajectory.images' | 'conversation.trajectory.action'>
  & InjectFace<TrajectoryViewInjected>
  & PropsLocale<'trajectory'>) {
  const rootRef = useRef<HTMLDivElement | null>(null)
  // The view mounts fresh on every chat→trajectory tab handover; a short
  // rise-and-fade reads as the ledger taking the stage. Owned gsap context,
  // motion-gated.
  useEffect(() => {
    if (!motionAllowed()) return undefined
    const ctx = gsap.context(() => {
      gsap.from(rootRef.current, { opacity: 0, y: 10, duration: 0.3, ease: 'power2.out' })
    })
    return () => { ctx.revert() }
  }, [])
  ...
  const timelineMode: TrajectoryTimelineMode = actualDuration
    ...
    : 'sequence'
  const toolbarMode: TrajectoryTimelineModeChoice = actualDuration
    ? actualTime ? 'actual' : 'duration'
    : 'equal'
  ...
  return (
    <div ref={rootRef} className={css.root} data-conversation-composer-overlay="">
      <TrajectoryToolbar
        mode={toolbarMode}
        onModeChange={(mode) => {
          setTimelineSelection(null)
          setActualDuration(mode !== 'equal')
          setActualTime(mode === 'actual')
        }}
        ...
        matchCount={searchMatchIndexes === null ? null : searchMatchIndexes.size}
        t={t}
        trailing={renderSlot('conversation.trajectory.action', {})}
      />
      <TrajectoryTimeline
        t={t}
        turns={timelineTurns}
