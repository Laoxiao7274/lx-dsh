...
describe('TrajectoryTable', () => {
  ...
  it('keeps the selected record when older rows shift projection indexes', () => {
    ...
    fireEvent.click(screen.getByRole('row', { name: /ASSISTANT, selected tail response/ }))
    ...
    expect(screen.getByRole('row', { name: /ASSISTANT, selected tail response/ })
  ...
  it('places the request boundary after leading steering input', () => {
    ...
    expect(request.closest('tr')?.getAttribute('aria-label')).toBe('Request #1, Step 2')
  })
  ...
  it('mounts only the visible window for a long ledger', async () => {
    ...
    expect(view.container.querySelector('tr[data-trajectory-row-key]')
      ?.getAttribute('aria-rowindex')).toBe('2')
    expect(scrollTo).toHaveBeenCalled()
    expect(view.container.querySelector('tr[data-virtual-spacer="bottom"]')).toBeTruthy()
    expect(screen.getByRole('row', { name: 'CONTEXT, Context 1' })).toBeTruthy()
    ...
    expect(screen.queryByRole('row', { name: 'CONTEXT, Context 1' })).toBeNull()
  })
  ...
  it('keeps the virtual tail reachable with collapsed-summary row heights', async () => {
    ...
    const tablePane = screen.getByRole('table').parentElement as HTMLElement
    tablePane.scrollTop = 100_000
    fireEvent.scroll(tablePane)

    await waitFor(() => {
      // The window must reach the ledger tail: every folded turn renders its
      // head strip, step row, and two collapsed rows, so the last rendered
      // virtual position sits deep in the three-hundreds.
      const rows = view.container.querySelectorAll('tr[data-virtual-position]')
      expect(rows.length).toBeGreaterThan(0)
      expect(Number(rows[rows.length - 1]?.getAttribute('data-virtual-position')))
        .toBeGreaterThan(300)
    })
  })
  ...
  it('keeps running and failure semantics distinct from record roles', () => {
    ...
    // The step chip also carries the failure label, so the record status is
    // one of several "Failed" texts; the error styling identifies the record's.
    const failedStatus = screen.getAllByText('Failed')
      .find(element => element.className.includes('error'))
  ...
  it('marks failed requests on their step chips', () => {
    const turns: readonly TrajectoryTurnModel[] = [
      {
        turn: 1,
        groups: [{
    ...
    expect(retry.getAttribute('data-request-status')).toBe('error')
    expect(recovered.hasAttribute('data-request-status')).toBe(false)
  })
  ...
  it('keeps a compact turn label available for narrow layouts', () => {
    ...
    expect(turnLabel.textContent).toContain('T1')
    expect(turnLabel.textContent).toContain('1 step · 2 calls')
  })
