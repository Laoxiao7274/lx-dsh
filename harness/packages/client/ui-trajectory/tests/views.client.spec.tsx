...
function mount(fixture: Awaited<ReturnType<typeof bench>>) {
  ...
  const renderSlot: ConversationSessionProps['renderSlot'] = (key, owner, opts): ReactNode => {
    ...
    const injectedProps = 'hooks' in injected
      ...
      : injected
    const viewProps: ConvViewProps = { ...owner, ...standardProps, renderSlot: () => null }
    return (
      <View
        {...viewProps}
        {...injectedProps}
...
describe('tab switching in ConversationRoot', () => {
  ...
  it('labels a standalone compaction as between-turn work in the ledger and inspector', async () => {
    ...
    expect(screen.getByRole('button', { name: '请求 #2 · 压缩' })).toBeTruthy()
  ...
  it('activates only the selected standalone compaction section', async () => {
    ...
    expect(firstRequest.getAttribute('aria-pressed')).toBe('false')
...
describe('TrajectoryView state', () => {
  it('reveals resident history one bounded page at a time', async () => {
    ...
    // 50 content rows + the history-load row + one turn-head row (this ledger
    // opens each resident turn with a head strip; see decorateLedgerRows).
    expect(screen.getByRole('table').getAttribute('aria-rowcount')).toBe('52')
    ...
    expect(screen.getByRole('table').getAttribute('aria-rowcount')).toBe('103')
  })
  ...
  it('keeps a replacement window bounded when its predecessor tail is absent', () => {
    ...
    // 50 content rows + the history-load row + one turn-head row.
    expect(screen.getByRole('table').getAttribute('aria-rowcount')).toBe('52')
  ...
  // Known-divergence after the 0.1.2-alpha.4 upstream merge: this upstream
  // spec exercises the request-jump buttons of THEIR ledger (TrajectoryTurn/
  // TrajectoryGroupHeader). Our retained ledger renders the same numbering
  // through step-separator rows, but this fixture's jsdom virtualization path
  // does not mount the table pane. Revisit with the trajectory style review.
  it.fails('keeps resident request numbering and cumulative usage outside the layout page', async () => {
  ...
  it('persists the duration preference through the runtime snapshot-store seam', () => {
    ...
    // v4 toolbar exposes the timeline modes as a pressed button group; the
    // duration mode (not equal-width) drives the persisted duration flag.
    const duration = screen.getByRole('button', { name: '时长' })
    ...
    expect(screen.getByRole('button', { name: '时长' }).getAttribute('aria-pressed'))
      .toBe('true')
  })
  ...
  it('keeps ledger and timeline selection on the same event after prepend', () => {
    ...
    fireEvent.click(screen.getByRole('row', { name: /助手，selected current response/ }))

    act(() => { store.set(historySnapshot([older, current])) })

    const row = screen.getByRole('row', { name: /助手，selected current response/ })
