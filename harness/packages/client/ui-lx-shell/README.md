...
- **User todos** (a `sidebar.workspaces.groupRow` first row inside every workspace group + a `sidebar.workspaces.groupBadge` count pill after every group header + a `sidebar.footer.action` rail fallback + `shell.overlay` panel, all optional behind the `todos` bridge member): the group row sits beside that project's sessions and opens that workspace's panel; the badge keeps the count visible on collapsed groups and opens the panel without expanding; `LxTodosEntry` is the rail icon fallback (cross-bucket badge). `LxTodosPanel` (anchored popover titled with the workspace, add via Enter, toggle done, remove, outside-click/Escape close). Lists are **bucketed per workspace** in `userData/todos.json` (the current session's workspace bucket, `''` default without one); the bridge carries the bucket key on every call plus a `counts()` badge feed pulled once at bind time. `todo-store.ts` (`createTodoPanelStore`) mirrors it for render, carrying the open anchor (inject results are memoized, so the live anchor rides the store). Older shells without the `todos` member register nothing.
