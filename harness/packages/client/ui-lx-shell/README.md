...
- **User todos** (a leading `sidebar.workspaces.leading` tree row beside the workspace folders + a `sidebar.footer.action` rail fallback + `shell.overlay` panel, all optional behind the `todos` bridge member): the tree row **expands into one count line per workspace** — every project's open count is visible without opening the project, and clicking a line opens that workspace's panel; the header badge shows only the CURRENT session's workspace count. `LxTodosEntry` is the rail icon fallback (current-workspace badge). `LxTodosPanel` (anchored popover titled with the workspace, add via Enter, toggle done, remove, outside-click/Escape close). Lists are **bucketed per workspace** in `userData/todos.json` (the current session's workspace bucket, `''` default without one); the bridge carries the bucket key on every call plus a `counts()` badge feed pulled once at bind time. `todo-store.ts` (`createTodoPanelStore`) mirrors it for render, carrying the open anchor (inject results are memoized, so the live anchor rides the store). Older shells without the `todos` member register nothing.
