...
- **User todos** (`sidebar.footer.action` entry + `shell.overlay` panel, both optional behind the `todos` bridge member): `LxTodosEntry` (open count badge, rail icon circle, passes its open-time rectangle to the panel) and `LxTodosPanel` (anchored popover: add via Enter, toggle done, remove, outside-click/Escape close). The list lives in the shell's `userData/todos.json` through the bridge (`get`/`add`/`remove`/`toggle` return the post-state list); `todo-store.ts` (`createTodoPanelStore`) mirrors it for render. Older shells without the `todos` member register nothing.
