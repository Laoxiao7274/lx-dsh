...
- **User todos** (a leading `sidebar.workspaces.leading` tree row beside the workspace folders + a `sidebar.footer.action` rail fallback + `shell.overlay` panel, all optional behind the `todos` bridge member): `LxTodosTreeRow` (badge summing open counts across buckets, passes its open-time rectangle and the current session's workspace) , `LxTodosEntry` (rail icon circle), and `LxTodosPanel` (anchored popover titled with the workspace, add via Enter, toggle done, remove, outside-click/Escape close). Lists are **bucketed per workspace** in `userData/todos.json` (the current session's workspace bucket, `''` default without one); the bridge carries the bucket key on every call plus a `counts()` badge feed. `todo-store.ts` (`createTodoPanelStore`) mirrors it for render, carrying the open anchor (inject results are memoized, so the live anchor rides the store). Older shells without the `todos` member register nothing.
