...
function changeRows(container: HTMLElement): string[] {
  return [...container.querySelectorAll('[class*="_del_"] > [class*="_lineText_"], [class*="_add_"] > [class*="_lineText_"]')]
    .map(row => row.textContent ?? '')
}

function footerText(container: HTMLElement): string {
  return container.querySelector('[class*="_footer_"]')?.textContent ?? ''
}
...
describe('DiffBlock structure', () => {
  ...
  it('numbers del rows on the old side and add rows on the new side', () => {
    // AICSS file-diff geometry: each change line carries its side's number in
    // the gutter; the other side's column stays empty for that row.
    const diffs: DiffHunk[] = [{ path: 'a.ts', oldText: 'one\ntwo', newText: 'one\nthree\nfour' }]
    const { container } = render(<DiffBlock diffs={diffs} />)
    const rows = [...container.querySelectorAll('[class*="_del_"], [class*="_add_"]')]
    // Two del rows carry old-side 1,2; three add rows carry new-side 1,2,3.
    expect(rows.map(row => row.querySelectorAll('[class*="_lineNo_"]')[0]?.textContent)).toEqual(['1', '2', '', '', ''])
    expect(rows.map(row => row.querySelectorAll('[class*="_lineNo_"]')[1]?.textContent)).toEqual(['', '', '1', '2', '3'])
  })
  ...
  it('treats a trailing newline as a terminator, not an extra blank line', () => {
    ...
    expect(footerText(container)).toBe('+1 −0 · 1 file')
  })
  ...
  it('renders a full deletion as removed-only with no phantom added line', () => {
    ...
    expect(footerText(container)).toBe('+0 −2 · 1 file')
  })
...
describe('DiffBlock footer', () => {
  it('counts added and removed lines and one file', () => {
    ...
    const { container } = render(<DiffBlock diffs={diffs} />)
    expect(footerText(container)).toBe('+1 −2 · 1 file')
  })
  ...
  it('pluralizes the distinct-file count', () => {
    ...
    const { container } = render(<DiffBlock diffs={diffs} />)
    expect(footerText(container)).toBe('+2 −0 · 2 files')
  })
})
