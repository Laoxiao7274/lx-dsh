...
目录添加之外，选择器的添加菜单还提供**连接远端后端…**：一个填写显示名、`ip:port` 地址和从目标 Host 远端访问设置行复制的访问令牌的对话框。保存后记录成为浏览器本地书签（保存在此浏览器；当前 Host 永远不持有指向另一个 Host 的连接），并在新标签页打开远端后端自己的 web UI，令牌放在查询串上。已保存的书签在选择器菜单中以地球图标列出，选中时以相同方式打开，并可从对话框的管理列表中移除。连接操作不需要目录流占位——没有选择器的组合仍然提供它。
