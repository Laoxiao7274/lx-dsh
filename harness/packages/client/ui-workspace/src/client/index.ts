...
import { createRemoteConnectionsStore, createWorkspaceViewStore } from './stores.ts'
...
export type { WorkspaceGroupRowOwnerProps } from './contract/slots.ts'
...
export function apply(ctx: Context): void {
  ...
  // One bookmark handle shared by every seat that renders remote connections
  // (the picker today): browser-local navigation records must not fork state
  // between surfaces, and a module-level handle would pin the store identity.
  const remoteConnections = createRemoteConnectionsStore()
  ...
  ctx.slots.inject('sidebar.workspaces', () => ctx.slots.register(
    {
      ...
      children: {
        ...
        'sidebar.workspaces.groupBadge': { kind: 'single', scope: 'root' },
