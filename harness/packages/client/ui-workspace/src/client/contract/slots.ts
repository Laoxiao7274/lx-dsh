...
import type { createRemoteConnectionsStore, createWorkspaceViewStore } from '../stores.ts'
...
/**
 * Full picker props: the owner share plus the creation callback, the
 * remote-connection bookmarks, and the locale seat. The two picker holes
 * (blank-session hero / New-Session view) share one owner currency, so one
 * composed type serves both registrations.
 */
export type WorkspacePickerProps =
  PropsRuntime<'conversation.hero.workspace'>
  & PropsRenderSlots<'conversation.hero.workspace.directoryFlow'>
  & PropsStore<ReturnType<typeof createRemoteConnectionsStore>>
