...
import { newRemoteBookmarkId } from './remote-connections.ts'
...
/** Browser state of the saved remote-backend connection bookmarks. */
type RemoteConnectionsState = {
  /** Saved bookmarks in creation order. */
 ...
 * return type); drift fails assignability at the defineStore call.
 */
type RemoteConnectionsActions = {
  addBookmark: (draft: RemoteConnectionsState, bookmark: Omit<RemoteBookmark, 'id' | 'createdAt'>) => void
  removeBookmark: (draft: RemoteConnectionsState, id: RemoteBookmarkId) => void
}

/**
 * Create the remote-backend bookmark store handle (persisted in this
 * browser; a connection record to a different backend is navigation data
 * the current backend must not own).
 * @returns the store handle (spec + type + identity + factory in one).
 */
export function createRemoteConnectionsStore(): EngineStoreHandle<RemoteConnectionsState, RemoteConnectionsActions> {
  return defineStore({
    init: (): RemoteConnectionsState => ({ bookmarks: [] }),
    persist: 'dsh.workspace.remote.v1',
    actions: {
      addBookmark: (d, bookmark) => {
        d.bookmarks = [...d.bookmarks, {
          ...bookmark,
          id: newRemoteBookmarkId(),
          createdAt: new Date().toISOString(),
        }]
      },
      removeBookmark: (d, id) => {
        d.bookmarks = d.bookmarks.filter(bookmark => bookmark.id !== id)
      },
    },
  })
}
