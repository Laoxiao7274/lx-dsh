...
Beside the directory add, the picker's add menu offers **Connect to a remote backend…**: a dialog for a display name, an `ip:port` address, and the access token copied from the target Host's remote-access settings row. Saving keeps the record as a browser-local bookmark (persisted in this browser; the current Host never owns a connection to a different one) and opens the remote backend's own web UI in a new tab with the token on the query string. Saved bookmarks list in the picker menu with a globe icon, open the same way on selection, and can be removed from the dialog's management list. The connect action needs no directory-flow occupant — a composition without a picker still offers it.
...
### The leading tree row

The browsing registration also declares a **leading row hole** (`sidebar.workspaces.leading`, `single` kind, wide column only — the rail hides the tree): its occupant renders as the first row of the tree, above every Workspace group and the Ungrouped bucket. The row is occupant-owned styling and behavior (an LX-DSH composition uses it for the user-todos entry); an empty hole renders nothing, and nothing in this package depends on it.
