...
// live, optional branch and rewind wiring, and an optional date-aware clock.

import { useCallback, useEffect, useId, useRef, useState, type ReactNode } from 'react'
import {
  Button, IconBranchOutline16, IconCheckOutline16, IconCopyOutline16, IconRefreshOutline16, Modal, Tooltip, writeClipboard,
...
export interface MessageIconActionsProps {
  ...
  /** Rewind the session in place to this user message; omission hides the rewind action. */
  onRewind?: (() => void) | undefined
  /** The session is compacted after this message, so rewind stays visible but unavailable. */
...
/**
 * Copy / branch / rewind (/ clock) IconActions row shared by user and assistant chrome.
 * @param props - Copy text, event time, clock side, branch and rewind callbacks, className.
 * @returns The actions row element.
 */
export function MessageIconActions({
  text, time, clock, onBranch, branchUnavailable = false, className,
  onRewind, rewindUnavailable = false,
  ...
  return (
    <div className={className === undefined ? css.actions : `${css.actions} ${className}`}>
      ...
      {extraActions}
      {onRewind !== undefined && (
        <Tooltip label={rewindUnavailable ? t('rewind.unavailable') : t('rewind')} side="bottom">
          ...
          <button
            ...
            onClick={rewindUnavailable ? undefined : () => { setRewindConfirm(true) }}
          >
      ...
      {onRewind !== undefined && rewindUnavailable && (
        <span id={rewindReasonId} className={css.visuallyHidden}>{t('rewind.unavailable')}</span>
      )}
      {onBranch !== undefined && (
      ...
      {rewindConfirm && onRewind !== undefined && (
        <Modal
          open
          onClose={() => { setRewindConfirm(false) }}
          closeLabel={t('rewind.cancel')}
          title={t('rewind.confirmTitle')}
          description={t('rewind.confirmMessage')}
          footer={(
            <>
              <Button variant="outline" className={css.modalAction} onClick={() => { setRewindConfirm(false) }}>
                {t('rewind.cancel')}
              </Button>
