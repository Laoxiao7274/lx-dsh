...
export class TestSessions implements ISessions {
  ...
  readonly calls: {
    method: 'create' | 'open' | 'openSubagent' | 'setSubagentCatalogOpen' | 'refreshSubagents'
      | 'clear' | 'refresh' | 'search' | 'fork' | 'rewind'
    args: unknown[]
  }[] = []
  ...
  /**
   * Recorded rewind stub: truncates nothing (benches asserting the full rewind
   * flow drive the production service; this face only proves the call).
   * @param opts - target session id and the event seq to cut back to.
   * @returns the transport-accepted ack.
   */
  rewind(opts: { sessionId: SessionId; atSeq: number }): Promise<{ ok: true; value: { accepted: true } }> {
    this.calls.push({ method: 'rewind', args: [opts] })
    return Promise.resolve({ ok: true, value: { accepted: true as const } })
  }
