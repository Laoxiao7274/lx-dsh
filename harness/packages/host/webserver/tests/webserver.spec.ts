...
async function loadComposition(port = 0, gzip = false, corsOrigins?: readonly string[]): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    ...(corsOrigins === undefined
      ? []
      : [`    corsOrigins: [${corsOrigins.map(o => `'${o}'`).join(', ')}]`]),
    '',
  ].join('\n'))
...
describe('real Loader composition', () => {
  it('applies gzip only to eligible socket-backed HTTP responses', { timeout: 60_000 }, async () => {
    expect(HttpServer.Config({ host: '127.0.0.1', port: 0, corsOrigins: [] })).toEqual({
      host: '127.0.0.1',
      port: 0,
      corsOrigins: [],
      compression: 'none',
    ...
    expect(() => HttpServer.Config({
      host: '127.0.0.1', port: 0, corsOrigins: [], compressionLevel: 10,
    })).toThrow()
  ...
  it('emits no CORS headers and answers any OPTIONS without an allowlist', { timeout: 60_000 }, async () => {
    const loaded = await loadComposition()
    const server = loaded.webServer
    server.register({
      kind: 'exact',
    ...
    const sameOrigin = await request(server.port, '/cors-text', {
      headers: { origin: 'https://elsewhere.example' },
    })
    expect(sameOrigin.status).toBe(200)
    expect(sameOrigin.headers.get('access-control-allow-origin')).toBeNull()
    const options = await request(server.port, '/cors-text', {
      method: 'OPTIONS',
      headers: { origin: 'https://elsewhere.example' },
    })
    // Without an allowlist the preflight falls through to the route owner.
    expect(options.status).not.toBe(204)
  })

  it('stamps exact-origin CORS headers, answers preflights, and rejects off-list upgrades', { timeout: 60_000 }, async () => {
    const allowed = 'https://client.example'
    const loaded = await loadComposition(0, false, [allowed])
    const server = loaded.webServer
    server.register({
      kind: 'exact',
      path: '/cors-text',
    ...
    const allowedRequest = await request(server.port, '/cors-text', {
      headers: { origin: allowed },
    })
    expect(allowedRequest.status).toBe(200)
    expect(allowedRequest.headers.get('access-control-allow-origin')).toBe(allowed)
    expect(allowedRequest.headers.get('access-control-allow-credentials')).toBe('true')
    expect(allowedRequest.headers.get('vary')).toContain('Origin')

    const unknownRequest = await request(server.port, '/cors-text', {
      headers: { origin: 'https://stranger.example' },
    })
    expect(unknownRequest.status).toBe(200)
    expect(unknownRequest.headers.get('access-control-allow-origin')).toBeNull()

    const preflight = await request(server.port, '/cors-text', {
      method: 'OPTIONS',
      headers: { origin: allowed },
    })
    ...
    // Off-list and absent origins are refused the upgrade; the allowlisted
    // origin completes the protocol switch.
    const refused = connect(server.port, '127.0.0.1')
    await once(refused, 'connect')
    refused.write([
      'GET /cors-ws HTTP/1.1',
    ...
    const [refusal] = await once(refused, 'data') as [Buffer]
    expect(String(refusal)).toContain('403')
    refused.destroy()

    const accepted = connect(server.port, '127.0.0.1')
    ...
    const [handshake] = await once(accepted, 'data') as [Buffer]
