...
describe('the shipped preset root', () => {
  it('supplies the built-in presets from a bare roster, healthy and system-trusted', async () => {
    ...
    expect(listed.map(preset => preset.id).sort())
      .toEqual(['cordis', 'lxcode', 'minimal', 'ptc', 'quick-answers', 'standard'])
  ...
  it('enables web_fetch in each tool-bearing Web app preset', async () => {
    for (const id of ['cordis', 'lxcode', 'ptc', 'quick-answers', 'standard']) {
  ...
  it('composes quick-answers as web-only: no shell, fs, or editor rows', async () => {
    const entries = await shippedEntries('quick-answers')
    ...
    expect(findEntry(entries, 'compaction-basic')).toBeTruthy()
    expect(findEntry(entries, 'tool-result-pruner')).toBeTruthy()
    // The search-grounded Q&A preset names none of the coding tools.
    for (const absent of ['tool-bash', 'tool-pwsh', 'tool-fs', 'tool-fs-search', 'str-replace-editor', 'tool-todo', 'tool-ask-user', 'persistent-shell', 'filesystem']) {
      expect(findEntry(entries, absent), absent).toBeUndefined()
    }
    // The persona is the complete system prompt with runtime context off.
    const persona = entries.find((entry: unknown) =>
      typeof entry === 'object' && entry !== null && 'id' in entry && (entry as { id: unknown }).id === 'persona')
    expect((persona as { config?: { complete?: boolean; includeRuntimeContext?: boolean } }).config)
      .toMatchObject({ complete: true, includeRuntimeContext: false })
  })
})
