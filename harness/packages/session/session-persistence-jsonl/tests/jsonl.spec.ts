...
describe('JsonlSessionPersistence: scanLog unit', () => {
  ...
  it('absorbs an unparsable row and keeps scanning rows that still continue the log', () => {
    ...
    expect(result.absorbedRows).toBe(1)
    // A later write is dropped once an unsalvageable hole stops the scan.
    const halted = new SessionLogScanner(header)
    halted.write(Buffer.from([
      JSON.stringify({ type: 'turn/end', seq: SessionSeq(1), time: 2, data: { turn: 1, reason: { kind: 'completed' } } }),
      '',
    ].join('\n')))
    expect(halted.finish().issue?.message).toMatch(/seq gap in committed region/)
    // The stopping hole discards everything after it: the scan consumed no
    // further rows, so the preserved prefix stays exactly the balanced start.
    expect(halted.finish().events).toHaveLength(0)
  })

  it('drops a stale-cursor row that re-covers already-scanned events', () => {
    const header = Buffer.from(`${JSON.stringify(toHeaderLine(meta('scanner-stale-row')))}\n`)
    const scanner = new SessionLogScanner(header)
    scanner.write(Buffer.from([
      JSON.stringify({ type: 'turn/start', seq: SessionSeq(0), time: 1, data: { turn: 1 } }),
      JSON.stringify({ type: 'turn/start', seq: SessionSeq(0), time: 5, data: { turn: 1 } }),
      JSON.stringify({ type: 'turn/end', seq: SessionSeq(1), time: 6, data: { turn: 1, reason: { kind: 'completed' } } }),
      '',
    ].join('\n')))
    const result = scanner.finish()
    expect(result.events.map(event => event.type)).toEqual(['turn/start', 'turn/end'])
    expect(result.absorbedRows).toBe(1)
    expect(result.issue).toBeUndefined()
  })

  it('replaces a foreign end-seed marker with the live writer row that re-covers its seq', () => {
    const header = Buffer.from(`${JSON.stringify(toHeaderLine(meta('scanner-marker-rewind')))}\n`)
    const scanner = new SessionLogScanner(header)
    scanner.write(Buffer.from([
      JSON.stringify({ type: 'turn/start', seq: SessionSeq(0), time: 1, data: { turn: 1 } }),
      JSON.stringify({ type: 'turn/end', seq: SessionSeq(1), time: 2, data: { turn: 1, reason: { kind: 'completed' } } }),
      // A foreign cold-open appended the marker at the live cursor...
      JSON.stringify({ type: 'session/end-seed', seq: SessionSeq(2), time: 3, data: {} }),
      // ...then the live writer's next row re-covers seq 2 with real content.
      JSON.stringify({ type: 'turn/start', seq: SessionSeq(2), time: 4, data: { turn: 2 } }),
      JSON.stringify({ type: 'turn/end', seq: SessionSeq(3), time: 5, data: { turn: 2, reason: { kind: 'completed' } } }),
      '',
    ].join('\n')))
    const result = scanner.finish()
    expect(result.events.map(event => `${event.type}@${event.seq}`).join(',')).toBe(
      'turn/start@0,turn/end@1,turn/start@2,turn/end@3',
    )
    expect(result.absorbedRows).toBe(1)
    expect(result.issue).toBeUndefined()
  })
  ...
  it('a seq gap after the last turn/end bounds the preserved tail (torn fragment tolerated)', () => {
    ...
    // No turn-opener bridge fits the hole, so the gap is a tolerated crash
    // boundary: scanLog PRESERVES the contiguous prefix (turn/start seq 0) —
    // real interrupted-turn work, not discarded — and reports the stopping hole.
    const scanned = scanLog(Buffer.from(log))
    expect(scanned.events.map(e => e.seq)).toEqual([0])
    expect(scanned.issue?.message).toMatch(/seq gap in committed region/)
  })

  it('a seq gap BEFORE a later committed turn/end still preserves the prefix (committed data damaged)', () => {
    const log = [
    ...
    // The read salvages the contiguous prefix instead of refusing the whole log.
    const scanned = scanLog(Buffer.from(log))
    expect(scanned.events.map(e => e.seq)).toEqual([0])
    expect(scanned.issue?.message).toMatch(/seq gap in committed region/)
  })

  it('salvages the prefix when malformed records precede a later committed turn/end', () => {
    ...
    for (const record of corruptRecords) {
      ...
      // The malformed row is absorbed; the following turn/end cannot continue
      // at seq 0, so the scan stops at that hole.
      expect(scanned.absorbedRows).toBe(1)
      expect(scanned.issue?.message).toMatch(/seq gap in committed region/)
    }
  })
...
describe('JsonlSessionPersistence: default packed chunk rows', () => {
  ...
  it('scanLog: a malformed packed row is absorbed like corrupt JSON', () => {
    const logText = [
      JSON.stringify({ type: 'session', version: 0, id: 'bad-row', createdAt: 1, delegationDepth: 0 }),
      // dt arity mismatch — row validation throws, so the line is unusable.
  ...
  it('scanLog: a packed row with a mid-run seq gap after the last turn/end drops the whole row', () => {
    const logText = [
      ...
      JSON.stringify({ type: 'turn/start', seq: SessionSeq(0), time: 1, data: { turn: 1 } }),
      // seq0 skips 1 — the run's first member is already a gap, and a chunk
      // row cannot follow synthetic turn closers, so the scan stops there.
