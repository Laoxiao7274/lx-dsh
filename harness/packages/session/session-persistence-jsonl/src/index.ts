...
interface StoredLog {
  ...
  /** Rows the read dropped as stale-writer overlap, unparsable content, or replaced markers. */
  readonly absorbedRows: number
...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  async readStoredLog(path: string, expectedId: SessionId, signal?: AbortSignal): Promise<StoredLog> {
    ...
    try {
      if (this.compression === 'zstd') {
        ...
        parsed = {
          ...
          // An unbridgeable committed-region hole or an uncommitted crash
          // fragment bounds the preserved prefix; absorbed stale rows do not
          // (the read tolerates them without rewriting the artifact).
          tornTruncateTo: scanned.issue !== undefined || scanned.tornTail ? scanned.committedBytes : undefined,
          // A torn raw tail is one incomplete JSONL line; it holds no complete
          // record to recover.
          recoveredTail: [],
          absorbedRows: scanned.absorbedRows,
        }
      }
    } catch (error: unknown) {
      signal?.throwIfAborted()
    ...
    // Stale-writer rows absorbed on read are the visible symptom of two
    // processes sharing this artifact: surface it instead of healing silently.
    if (parsed.absorbedRows > 0) {
      this.ctx.logger.warn(
        `${this.name}: session "${expectedId}" read tolerating ${parsed.absorbedRows} stale log row(s) `
        + `written by a divergent cursor — two backends may be sharing this storage root (raw log: ${path})`,
      )
    }
  ...
  private async readZstdPrefix(
    ...
    signal?.throwIfAborted()
    const { frames, tornStart, corruptStart } = scanZstdFramesSalvage(buffer)
    signal?.throwIfAborted()
    ...
    try {
      ...
      // A durable cut that must stop the scan: the byte offset to truncate the
      // artifact to, plus the event count scanned before the cut frame — its
      // accepted events re-append after truncation through the write seam.
      let cut: { truncateTo: number; eventCount: number } | undefined
      let frameIndex = 0
      for (const plaintext of decodedFrames) {
        frameIndex += 1
        const beforeFrame = scanner.checkpoint()
        scanner.write(plaintext)
        const holeFrame = frames[frameIndex]
        if (scanner.issueError() !== undefined && holeFrame !== undefined) {
          cut = { truncateTo: holeFrame.start, eventCount: beforeFrame.eventCount }
          break
        }
        if (frameIndex < frames.length - 1 && performance.now() >= yieldDeadline) {
      ...
      if (cut !== undefined) {
        ...
        return {
          ...
          tornTruncateTo: cut.truncateTo,
          recoveredTail: prefix.events.slice(cut.eventCount),
          absorbedRows: prefix.absorbedRows,
        }
      }
      // Structurally corrupt bytes after the last complete frame: everything
      // before them is the preserved prefix, and nothing after is recoverable.
      if (corruptStart !== undefined) {
        const prefix = scanner.finish()
        return {
          meta: prefix.meta,
          inheritedEventCount: prefix.inheritedEventCount,
          events: prefix.events,
          tornTruncateTo: corruptStart,
          recoveredTail: [],
          absorbedRows: prefix.absorbedRows,
        }
      }
      const complete = scanner.checkpoint()
      if (scanner.hasTrailingFragment()) {
      ...
      // the write path truncate the torn bytes and rewrite them durably. A hole
      // inside the recovered plaintext stops the scan; the recovered rows up
      // to it still re-append after the torn frame is truncated away.
      ...
      const recovered = scanner.finish()
      return {
        meta: recovered.meta,
        inheritedEventCount: recovered.inheritedEventCount,
        events: recovered.events,
        tornTruncateTo: tornStart,
        recoveredTail: recovered.events.slice(complete.eventCount),
        absorbedRows: recovered.absorbedRows,
      }
    } catch (error) {
      /* v8 ignore next -- decoder failure plus concurrent abort is timing-dependent */
      if (signal?.aborted) signal.throwIfAborted()
