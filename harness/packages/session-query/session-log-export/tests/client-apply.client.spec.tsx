...
function declare(slots: SlotRegistry): () => void {
  return slots.register({
    name: 'root',
    children: {
      'conversation.trajectory.action': { kind: 'list', scope: 'session' },
    },
  } as never, () => null)
}
...
describe('session-log-download browser plugin', () => {
  it('provides one controller and removes its Trajectory contribution on disposal', async () => {
    ...
    const entry = b.slots.entries('conversation.trajectory.action')[0]
    ...
    expect(b.slots.entries('conversation.trajectory.action')).toHaveLength(0)
  })
  ...
  it('re-registers after the declaring Trajectory slot collapses and returns', async () => {
    ...
    expect(b.slots.entries('conversation.trajectory.action')[0]?.component).toBe(SessionLogDownloadHeaderAction)
    redeclare()
    await b.fiber.dispose()
  })
})
