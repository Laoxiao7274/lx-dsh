...
function parseRemoteEventReady(value: unknown): {
  ...
  if (!isRemoteEventRecord(value)
    ...
    || !hasExactRemoteEventKeys(value.host, ['home', 'protocolVersion'])
    || typeof value.host.home !== 'string'
    || typeof value.host.protocolVersion !== 'number') {
    throw new TypeError('client api: forwarded Remote event stream did not begin with ready')
  }
  if (value.host.protocolVersion !== REMOTE_PROTOCOL_VERSION) {
    throw new TypeError(
      `client api: remote protocol version ${String(value.host.protocolVersion)} does not match this client (${String(REMOTE_PROTOCOL_VERSION)}); update the client or the backend`,
    )
  }
  return { clientId: value.clientId, host: { home: value.host.home, protocolVersion: value.host.protocolVersion } }
}
