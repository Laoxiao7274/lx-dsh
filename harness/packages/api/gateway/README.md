...
The Host entry also serves the contract at `GET /api/$schema`: one authenticated request returns the wire protocol version, the RPC path shape, the WebSocket mux path, and every locally registered invocation grouped by namespace with its wire parameter names, JSON-or-lookup source, and stable type symbols. A third-party client can be written against the running Host from that document alone; the snapshot is cached until the local registry changes. `scripts/api-only-smoke.mjs` boots a real backend and drives the full surface — Bearer-authenticated schema fetch, an RPC call built from the schema's own parameter names, and the mux ready frame — with no browser or static frontend involved.
...
Host composition can register one application event source through `registerRemoteEvents()`. Gateway reserves the internal `$events` logical endpoint for that source, accepts only empty `args`, and aborts streams opened by the registration when the source is withdrawn. API Remotes owns the event selection, argument validation, per-Client queues, and the Host facts sent in the opening `{ type: 'ready', clientId, host: { home, protocolVersion } }` frame — the Client refuses a generation whose wire version differs from its own with an explicit protocol-version error. Its source factory attaches incremental listeners synchronously, so the Client publishes the generation and starts baseline reads only after incremental delivery is ready.
