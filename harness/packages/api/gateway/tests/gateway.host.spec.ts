...
class FakeConnectionService extends Service {
  ...
  get fetch() {
    const owner = this.ctx
    return {
      register: (route: { path: string, methods: readonly string[], fetch: () => Promise<Response> }) =>
        owner.effect(() => {
          this.fetchRoutes.push(route)
          return () => {
            const at = this.fetchRoutes.indexOf(route)
            if (at !== -1) this.fetchRoutes.splice(at, 1)
          }
        }),
    }
  }

  readonly fetchRoutes: { path: string, methods: readonly string[], fetch: () => Promise<Response> }[] = []
...
describe('TypertGatewayService', () => {
  ...
  it('claims and validates in-process Remote event results for the active Client generation', async () => {
    ...
    })(), { home: '/home/fixture', protocolVersion: 1 })
    ...
    expect(opening).toMatchObject({
      ...
      value: { type: 'ready', host: { protocolVersion: 1, home: '/home/fixture' } },
    })
