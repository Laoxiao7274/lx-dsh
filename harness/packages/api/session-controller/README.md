...
`ClientSessions` persists the current-session selection to localStorage under `dsh.sessions.current`, and every same-origin window of one browser profile follows it: a `storage` event carrying a validated foreign selection re-targets this client through the same `open()`/`openSubagent()`/`clear()` paths a sidebar click takes, with a target not yet addressable here staying pending until its list row (or catalog entry) arrives. Windows of one profile therefore stage the same session and stream it live; different profiles keep independent selections. See the [cross-window selection Agent Note](../../.agents/notes/implemented/architecture/2026-09-02-cross-window-selection-follow.md).
