...
const AVAILABLE_STREAM_CONNECTION = {
  generation: {
    getSnapshot: () => ({ id: 1, host: { home: '/h', protocolVersion: 1 } }),
    subscribe: () => () => {},
  },
}
...
export class FakeApiClient {
  ...
  onRewind: (payload: unknown) => Promise<RemoteResult<{ accepted: true }>> = () => Promise.resolve(ok({ accepted: true }))
  ...
  onReadWorkspaceFile: (payload: unknown) => Promise<RemoteResult<SessionReadWorkspaceFileValue>> =
    () => Promise.resolve(ok({ kind: 'text', text: '', byteSize: 0, truncated: false }))
  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      session: {
        ...
        rewind: payload => this.record('session.rewind', payload, this.onRewind(payload)),
