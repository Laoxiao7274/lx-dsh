...
`ClientSessions` 把当前会话选择持久化到 localStorage 的 `dsh.sessions.current` 键，同一浏览器 profile 的所有同源窗口都会跟随它：携带已验证外来选择的 `storage` 事件会经由与侧栏点击完全相同的 `open()`/`openSubagent()`/`clear()` 路径重定向本客户端，本窗口尚不可寻址的目标保持 pending，直到其列表行（或目录条目）到达。因此同一 profile 的多个窗口 stage 同一会话并实时流式；不同 profile 保持独立选择。见[跨窗口选择 Agent Note](../../.agents/notes/implemented/architecture/2026-09-02-cross-window-selection-follow.md)。
