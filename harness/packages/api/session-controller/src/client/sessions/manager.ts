...
export class SessionManager {
  ...
  */
  ...
  /**
   * Contract rewind; on success the host has truncated the durable log and
   * checkpointed it, so the resident conversation window must reload its
   * history from the truncated state.
   * @param opts - session id and the `user/message` boundary to rewind from.
   * @returns the rewind result.
   */
  async rewind(
    opts: { sessionId: SessionId; atSeq: number },
  ): Promise<RemoteResult<{ accepted: true }>> {
    const result = await this.remote.session.rewind({
      sessionId: opts.sessionId,
      atSeq: opts.atSeq,
    })
    if (result.ok) {
      // Reload the conversation window so the UI reflects the truncated log.
      const session = this.sessions.get(opts.sessionId)
      if (session !== undefined) void session.resync()
    }
    return result
  }
