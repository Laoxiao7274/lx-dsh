...
export interface ISessions {
   ...
   * @param opts - target workspace, directory, optional preallocated
   * identity, and the agent preset the Session must run.
   ...
   * the turn boundary before `atSeq`'s user message. On success the resident
   * conversation window reloads from the truncated log, so the conversation
   * returns to the state before the message was sent.
   * @param opts - session id and the `user/message` event seq to rewind from.
   * @returns acknowledgement that the rewind was applied.
   */
  rewind(opts: { sessionId: SessionId; atSeq: number }): Promise<RemoteResult<{ accepted: true }>>
  /**
