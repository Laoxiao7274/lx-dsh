...
export interface Config {
  ...
  /**
   * Sessions one `list()` call may backfill in the background when their row
   * served without a projection block (title recovery through the projection
   * cache). 0 disables the backfill.
   */
...
export class SessionController extends TypertRemoteService {
  ...
  constructor(ctx: Context, config: Config, internals: SessionControllerInternals = {}) {
    ...
    this.listState = new ApiSessionList(ctx, {
      ...
      coldBackfillLimit: config.coldBackfillLimit ?? DEFAULT_COLD_BACKFILL_LIMIT,
    })
  ...
  /**
   * Rewind one live Session in place: truncate the durable log at the
   * boundary's turn so the conversation returns to before the message.
   * @param request - live Session and the `user/message` boundary.
   * @returns acknowledgement that the truncation was persisted.
   */
  @Remote('rewind')
  rewind(request: SessionRewindRequest): Promise<SessionRewindValue> {
