...
/** Session rewind request: truncate the live log to the prefix ending at the boundary. */
export interface SessionRewindRequest {
  /** Live Session to rewind in place. */
  readonly sessionId: SessionId
  /** Inclusive boundary: a `user/message` event seq whose whole turn is removed. */
  readonly atSeq: number
}

/** Acknowledgement that the durable log was truncated and checkpointed. */
