...
describe('the settings Remote namespace a configuration page calls', () => {
  it('publishes the settings namespace from its own service key', async () => {
    ...
    expect(remoteMethods(controller)).toEqual([
      ...
      { method: 'connectionInfo', invocation: { kind: 'direct' } },
  ...
  it('reports the authenticated URL, LAN addresses, and loopback posture for remote access', async () => {
    const loopback = new Context()
    loopback.provide('webRuntime', { lanAddresses: [] } as never)
    loopback.provide('webServer', { host: '127.0.0.1', port: 3080 } as never)
    loopback.provide('connection', {
      authenticatedUrl: (url: string) => `${url}/?token=launch-secret`,
    } as never)
    const loopbackController = new SettingsController(loopback)
    expect(loopbackController.connectionInfo()).toEqual({
    ...
    exposed.provide('webRuntime', { lanAddresses: ['192.168.1.5'] } as never)
    exposed.provide('webServer', { host: '0.0.0.0', port: 3080 } as never)
    exposed.provide('connection', {
      authenticatedUrl: (url: string) => `${url}/?token=launch-secret`,
    } as never)
    const exposedController = new SettingsController(exposed)
    expect(exposedController.connectionInfo()).toEqual({
      url: 'http://127.0.0.1:3080',
      token: 'launch-secret',
    ...
    // A hand-built tree with no mounted webserver/connection reports the
    // composition gap instead of crashing on a property access.
    const bare = new SettingsController(new Context())
    expect(() => { bare.connectionInfo() }).toThrow(RemoteError)
    expect(() => { bare.connectionInfo() }).toThrow('the composed backend serves no browser connection')

    const withoutServer = new Context()
    withoutServer.provide('connection', {
      authenticatedUrl: (url: string) => url,
    } as never)
    const serverlessController = new SettingsController(withoutServer)
    expect(() => { serverlessController.connectionInfo() }).toThrow('the composed backend serves no webserver')
  })
