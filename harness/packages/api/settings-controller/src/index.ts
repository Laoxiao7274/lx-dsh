...
// Type-only: pulls the `connection` and `webServer` Context declarations.
...
import type {
  AgentPresetDirectoryOpenValue, ConnectionInfoValue, SettingsDocumentOpenValue,
} from './types.ts'
...
export class SettingsController extends TypertRemoteService {
  ...
  /**
   * Describe how a remote client reaches this Host: the clean root address,
   * the launch token to paste beside it in a connect form, LAN addresses
   * for an all-interfaces bind, and whether the server is loopback-only.
   * Served inside an authenticated session — the same sensitivity as the
   * printed startup URL.
   * @returns the connection facts for the remote-access settings row.
   */
  @Remote
  connectionInfo(): ConnectionInfoValue {
    // ctx.get, not the property proxy: connection and webServer are not
    // declared injections of this service, and property access throws
    // outside an inject context.
    const connection = this.ctx.get('connection')
    if (connection === undefined) {
      throw new RemoteError('gateway/internal', 'the composed backend serves no browser connection', {})
    }
    const webServer = this.ctx.get('webServer') as
      | { host: string, port: number }
      | undefined
    if (webServer === undefined) {
      throw new RemoteError('gateway/internal', 'the composed backend serves no webserver', {})
    }
    const webRuntime = this.ctx.get('webRuntime') as
      | { lanAddresses?: readonly string[] }
      | undefined
    const loopbackOnly = webServer.host === '127.0.0.1'
    const root = `http://127.0.0.1:${String(webServer.port)}`
    // The authenticated URL carries the token on its query string; the row
    // presents address and token as separate copyable facts, so both come
    // out of the one canonical URL the connection mints.
    const authenticated = new URL(connection.authenticatedUrl(root))
    return {
      url: root,
      token: authenticated.searchParams.get('token') ?? '',
