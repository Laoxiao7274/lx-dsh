...
`settings.connectionInfo()` reports how a remote client reaches this Host: the authenticated root URL (the same launch token the printed startup URL carries), LAN IPv4 literals when the webserver binds all interfaces, and whether the server is loopback-only. It is served inside an authenticated session — the same sensitivity as the startup URL — and backs the General settings page's remote-access row, which copies the address out for a client on another machine.
