...
import { deriveEventMessage, isReplacementSurfaceEvent, SurfaceManager } from './surface.ts'
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
    ...
    /**
     * Awaited parallel durability checkpoint after a rewind: the in-memory log
     * was truncated to `atSeq` (inclusive) and every listener must truncate its
     * durable store to match. Scope-filtered dispatch reuses the session's owner scope.
     * @param session - the session whose log was truncated.
     * @param atSeq - the last event seq retained (inclusive boundary).
     * @dshScopeScan unsupported
     * @mode parallel
     */
    'session/truncate'(this: Scoped<Session>, session: Session, atSeq: number): Promise<void> | void
  }
}
...
export class Session {
  ...
  private surfaceManager = new SurfaceManager(this.log)
  ...
  /**
   * Truncate the event log to the prefix ending at `atSeq` (inclusive),
   * discarding every event after it. The session then continues from that
   * point: the next `append` reuses the seq that was `atSeq + 1` before
   * the rewind.
   *
   * Pre-release escape hatch from the append-only model: the caller (the
   * host API layer) is responsible for persisting the truncation and
   * notifying connected clients to re-fetch history. This method only
   * mutates the in-memory log and resets derived state.
   *
   * @param atSeq - the user/message event to rewind FROM. The rewind finds the
   *   `turn/start` that opened the turn containing this message and removes the
   *   entire turn (turn/start, user message, agent response), returning the
   *   conversation to the state before the user sent the message.
   * @throws when `atSeq` is out of range, does not identify a `user/message`
   *   event, or when a surface replacement (compaction) exists after `atSeq`
   *   — removing a compaction's replacement would leave the surface in an
   *   inconsistent state, so such a rewind is refused.
   */
  rewind(atSeq: number): void {
    if (!Number.isSafeInteger(atSeq) || atSeq < 0 || atSeq >= this.log.length) {
      throw new Error(`rewind boundary ${String(atSeq)} is out of range for session "${this.id}" (log length ${String(this.log.length)})`)
    }
    const boundary = this.log[atSeq]
    if (boundary === undefined || boundary.seq !== atSeq) {
      throw new Error(`rewind boundary ${String(atSeq)} does not match a contiguous event seq in session "${this.id}"`)
    }
    if (boundary.type !== 'user/message') {
      throw new Error(`rewind boundary ${String(atSeq)} in session "${this.id}" must be a user/message event, got "${boundary.type}"`)
    }
    // Find the turn/start that opened the turn containing this user message.
    // The rewind removes the ENTIRE turn (turn/start + user message + agent
    // response), so the conversation goes back to before the user sent it.
    let cutSeq = atSeq
    for (let i = atSeq; i >= 0; i--) {
      const event = this.log[i]
      if (event !== undefined && event.type === 'turn/start') {
        cutSeq = i
        break
      }
    }
    // Refuse if any surface replacement (compaction) exists after the cut:
    // removing the replacement would strand the shadowed nodes without their
    // summarizing replacement, corrupting the derived message history.
    for (let i = cutSeq; i < this.log.length; i++) {
      const event = this.log[i]
      if (event !== undefined && isReplacementSurfaceEvent(event)) {
        throw new Error(`session "${this.id}" has a surface replacement at seq ${String(event.seq)} after the rewind boundary — cannot rewind past a compaction`)
      }
    }
    // Truncate the log in place: remove the turn/start, user message, and
    // everything after it. The conversation returns to the state before the
    // user sent the message.
    this.log.length = cutSeq
    // Rebuild the surface manager over the truncated log.
    this.surfaceManager = new SurfaceManager(this.log)
    // Invalidate every derived cache so the next read rebuilds from scratch.
    this.eventsSnapshot = undefined
...
export class SessionStore extends Service {
  ...
  async flush(session: Session): Promise<boolean> {
    ...
    const results = await Promise.allSettled(callbacks.map((callback) => {
      try {
        ...
        // A synchronous throw converts to a rejected promise so the map keeps
        // invoking later listeners — one bad listener cannot starve the rest.
  ...
  /**
   * Truncate a live session's event log to the prefix ending at `atSeq`
   * (inclusive), discarding every event after it. The session keeps its id
   * and header; the next append reuses the seq that was `atSeq + 1`.
   *
   * Validates the boundary via {@link Session.rewind} (must be a
   * `user/message` event, and no surface replacement may exist after it).
   * Emits `session/truncate` so persistence plugins can truncate their
   * durable stores, then awaits `session/flush` to checkpoint.
   *
   * @param sessionId - the live session to rewind.
   * @param atSeq - inclusive boundary: the last event seq to keep.
   * @throws when the session is not live, the boundary is invalid, or a
   *   compaction exists after the boundary.
   */
  async rewind(sessionId: SessionId, atSeq: number): Promise<void> {
    const session = this.get(sessionId)
    if (session === undefined) throw new SessionForkError(`session "${sessionId}" not found`, 'SESSION_NOT_FOUND')
    session.rewind(atSeq)
    const { carrier } = this.liveEntryFor(session)
    const callbacks = collectSessionCallbacks(this.ctx, [carrier, 'session/truncate', session, atSeq])
    await Promise.allSettled(callbacks.map((callback) => {
      try { return callback(session, atSeq) } catch (error) { throw error }
    }))
