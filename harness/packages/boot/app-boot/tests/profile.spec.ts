...
describe('loadProfile', () => {
  ...
  it('migrates the retired stock web tuple to the shipped template and leaves richer lists untouched', () => {
    const anchor = stageInstallation({
      '@deepseek-ai/dsh-base': { patch: '[]\n' },
      '@deepseek-ai/dsh-web-app': { patch: '[]\n' },
      '@laoxiao7274/dsh-web-search': { patch: '[]\n' },
      '@liustack/modlens': { patch: '[]\n' },
      'user-installed-bundle': { patch: '[]\n' },
    })
    // The stock web tuple from before web-search and modlens joined the
    // template: normalized to the shipped template on load.
    const stockHome = tmp()
    ...
    // A list with any extra bundle is user-owned and remains untouched,
    // whether the extras came from `dsh plugin` or hand editing.
    const ownedHome = tmp()
    const owned = resolveProfileDir('web', ownedHome)
    initProfile(owned, [
      '@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app', 'user-installed-bundle',
    ])
    loadProfile('t', 'web', anchor, ownedHome)
    expect(readProfileManifest('t', owned).dsh?.profile?.bundles).toEqual([
      '@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app', 'user-installed-bundle',
    ])
  })
