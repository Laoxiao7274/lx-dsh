...
export const PROFILE_TEMPLATES: Record<string, ProfileTemplate> = {
  ...
  web: {
    // The two non-official bundles ride the installation's dependency closure
    // (web-app declares them), so the launcher's module fallback links them
    // into every profile without network access: the web search provider and
    // the ModLens vision bridge. They remain bundle-list entries, not
    // dependencies, so `dsh plugin` never reconciles them away.
    bundles: [
...
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  ...
  // The stock web tuple before the web-search and ModLens bundles shipped in
  // the template: existing installations migrate to the current template on
  // their next profile load. Profiles with any other bundle list (user-edited
  // or plugin-installed) are user-owned and remain untouched.
