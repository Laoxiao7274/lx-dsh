...
**使用现有 DeepSeek 设计系统，渲染保留轮次结构的紧凑事件记录表，并提供局部记录检查器。** 当前视觉语言是[轨迹页 v4](../architecture/2026-08-31-trajectory-v4-trace-explorer.zh.md)所描述的调研对齐执行轨迹浏览器;下文的行机制仍然为准。

- 记录表在已加载窗口内按 Session Event 顺序展示物化后的业务记录。每个轮次以一条轮次头横条(`T{n}` 方块 chip、首个用户可见标题、步数/调用/耗时/token chip)开始;每个请求分组以携带时间序请求编号的步骤分隔小注开始,普通请求与压缩(compaction)请求共用一套编号。
- 事件列是一条树轨加肘形连接线,每行一枚描边类型图标方块;内容列承载名称与参数,其后跟一段小号灰字的度量尾巴(耗时、token)。嵌套子工具略微缩进,内容预览使用 CSS 截断以适应可用宽度。
