 ...
 * and accepted for this repository's distribution. Each value records the
 * reviewed terms. This does not reclassify the terms as permissive — the
 * notices table discloses the declared license verbatim.
 */
export const REVIEWED_RUNTIME_TERMS: Record<string, string> = {
  // GSAP's standard license permits no-charge use and redistribution as part
  // of a larger work (the post-2024 Webflow terms); the bundled copy ships in
  // the client shell and in dynamic plugin bundles.
  'gsap': 'https://gsap.com/standard-license',
  // lightningcss is MPL-2.0: file-level copyleft. The harness consumes it as
  // an unmodified npm dependency (no source patches) from packages/client/ui-theme,
  // where it parses and scopes skin CSS host-side; MPL §3/§4 permit unmodified
  // binary distribution with the license text and source availability notice
  // carried in THIRD_PARTY_NOTICES, which this generator emits verbatim.
  'lightningcss': 'https://github.com/parcel-bundler/lightningcss/blob/master/LICENSE',
}
...
export function render(): string {
  ...
  const nonPermissiveRuntime = runtimeDeps.filter(dep =>
    ...
    && REVIEWED_RUNTIME_TERMS[dep.name] === undefined,
  )
