...
// Decorative client motion (gsap entrances/exits) stays off under test: specs
// assert structure, not tweens, and jsdom never ticks the animation frames.
// The flag -- not a matchMedia stub -- so consumers reading reduced motion for
// scroll behavior (AttachmentRail pageBehavior) keep their unit-lane contract.
if (typeof window !== 'undefined') {
  (window as unknown as { __DSH_TEST_MOTION_OFF__?: boolean }).__DSH_TEST_MOTION_OFF__ = true
}
...
// eslint-disable-next-line ts/unbound-method -- re-bound via .call(this) at every call site below.
