...
import type { RemoteAccessRowInjected } from './RemoteAccessRow.tsx'
import { SettingsDocumentAction } from './SettingsDocumentAction.tsx'
import type { SettingsDocumentActionInjected } from './SettingsDocumentAction.tsx'
import { SettingsDocumentStore } from './settings-document-store.ts'
import { RemoteAccessStore } from './remote-access-store.ts'
...
export type { RemoteAccessRowInjected, RemoteAccessRowProps } from './RemoteAccessRow.tsx'
export type { RemoteAccessState } from './remote-access-store.ts'
export { SettingsDocumentStore } from './settings-document-store.ts'
export { RemoteAccessStore } from './remote-access-store.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  // The remote-access row gates the Host's connection facts behind a reveal
  // switch: the URL carries the launch token, so it stays hidden until the
  // user asks for it. The store is loopback-hosted reading only; remote
  // backends answer it the same way.
  const remoteAccess = new RemoteAccessStore(ctx)
  ctx.effect(() => () => { remoteAccess.dispose() }, 'ui-settings-general: remote access store')
  const remoteAccessInjected = (): RemoteAccessRowInjected => ({
    hooks: { remoteAccess: remoteAccess.store },
    toggleRemoteAccess: () => { remoteAccess.toggle() },
  })
