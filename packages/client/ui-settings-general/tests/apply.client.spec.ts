...
describe('ui-settings-general apply', () => {
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    // This plugin seats the remote-access row itself; feature rows join it.
    expect(before.slots.entries('settings.general.item').map(e => e.options.id)).toEqual(['remote-access'])
  ...
  it('re-registers after an HMR collapse of the declaring chain (stale disposers must not block)', async () => {
    ...
    // The remote-access item registration rides the same recovery.
    await vi.waitFor(() => {
      expect(b.slots.entries('settings.general.item').map(e => e.options.id)).toEqual(['remote-access'])
    })
