...
import { createRemoteConnectionsStore } from '../src/client/stores.ts'
...
function mount(
  ...
  const remoteStore = createRemoteConnectionsStore().create()
  const renderPicker = (nextItems: readonly WorkspaceView[]) => (
    <WorkspacePicker
      ...
      useStore={bindSnapshotSelector(remoteStore)}
      actions={remoteStore.actions}
  ...
  return {
    view, onPick, onClose, createWorkspace, probe, occupancy, remoteStore,
...
describe('WorkspacePicker', () => {
  ...
  it('offers both add actions from the anchor gesture when nothing is listed', () => {
    // The remote-connect entry keeps the directory add company, so the
    // gesture opens the menu (two choices) instead of collapsing into the
    // flow; picking the directory add still raises it.
    const b = mount([])
    expect(screen.getByRole('menuitem', { name: '添加工作区…' })).toBeTruthy()
    expect(screen.getByRole('menuitem', { name: '连接远程后端…' })).toBeTruthy()
    fireEvent.click(screen.getByRole('menuitem', { name: '添加工作区…' }))
    expect(b.onClose).toHaveBeenCalled()
    expect(screen.getByTestId('directory-flow')).toBeTruthy()
  })
  ...
  it('waits to show its menu until an optional anchor is available', () => {
    ...
    render(
      <WorkspacePicker
        ...
        useStore={bindSnapshotSelector(remoteStore)} actions={remoteStore.actions}
  ...
  it('offers only the remote connect when nothing is listed and no flow is composed', () => {
    // A composition mounting this package without any directory-picker has
    // no Workspace to pick and no way to add one, but the remote connect
    // needs no flow — so the popover still offers exactly that one choice.
  ...
  it('saves and opens a remote connection, lists it, and removes it', () => {
    const openSpy = vi.spyOn(window, 'open').mockImplementation(() => null)
    try {
      const b = mount([workspace('alpha', 'Alpha')])
      fireEvent.click(screen.getByRole('menuitem', { name: '连接远程后端…' }))
      expect(screen.getByRole('dialog', { name: '连接远程后端' })).toBeTruthy()
      // An unparsable address keeps the dialog open with the validation copy.
      fireEvent.change(screen.getByPlaceholderText('例如：192.168.1.5:3080'), {
        target: { value: 'not a url' },
      })
      fireEvent.click(screen.getByRole('button', { name: '保存并打开' }))
      expect(screen.getByRole('alert').textContent).toBe('地址无法解析；请填写 IP 或域名，可带端口。')
      expect(b.remoteStore.getSnapshot().bookmarks).toEqual([])
      // A valid address saves the bookmark, closes the dialog, and opens the
      // remote backend's own UI with the token on the query string.
      fireEvent.change(screen.getByPlaceholderText('例如：192.168.1.5:3080'), {
        target: { value: '192.168.1.5:3080' },
      })
      fireEvent.change(screen.getByPlaceholderText('目标后端“外网访问”设置里复制的令牌'), {
        target: { value: 'secret-token' },
      })
      fireEvent.click(screen.getByRole('button', { name: '保存并打开' }))
      expect(b.remoteStore.getSnapshot().bookmarks).toEqual([
        expect.objectContaining({
          title: '192.168.1.5:3080', origin: 'http://192.168.1.5:3080', token: 'secret-token',
        }),
      ])
      expect(openSpy).toHaveBeenCalledWith('http://192.168.1.5:3080/?token=secret-token', '_blank', 'noopener')
      // The saved connection lists as a menu row and opens by selection.
      fireEvent.click(screen.getByRole('menuitem', { name: '192.168.1.5:3080' }))
      expect(openSpy).toHaveBeenCalledTimes(2)
      // The management list removes the bookmark.
      fireEvent.click(screen.getByRole('menuitem', { name: '连接远程后端…' }))
      fireEvent.click(screen.getByRole('button', { name: '移除此连接' }))
      expect(b.remoteStore.getSnapshot().bookmarks).toEqual([])
    } finally {
      openSpy.mockRestore()
    }
