...
import { formatElapsedSeconds, formatHeadDuration, trajectoryRecordId } from './trajectory-record.ts'
import { trajectoryRowMetrics } from './trajectory-row-metrics.ts'
import {
  decorateLedgerRows, formatTokenSummary,
  type StructuralRowFields,
} from './trajectory-structure-rows.ts'
import {
...
interface TableRecord extends StructuralRowFields {
  turn: number | null
  section: number
  group: string
  groupStart: boolean
...
/** Compact structural duration for head/step strips lives in trajectory-record. */
...
function AssistantTimingPanel({
  ...
  const started = metrics.stepStartTime
  const firstToken = metrics.firstTokenTime
  const completed = metrics.completedTime
  const ttftMs = started !== null && firstToken !== null && firstToken >= started
    ? firstToken - started
    : null
  const generationMs = firstToken !== null && completed !== null && completed >= firstToken
    ? completed - firstToken
    : null
  const totalMs = ttftMs !== null && generationMs !== null ? ttftMs + generationMs : null
  const ttftPercent = ttftMs === null || totalMs === null || totalMs <= 0
    ? null
    : Math.round(ttftMs / totalMs * 100)
  return (
    <>
      {ttftPercent !== null && ttftMs !== null && generationMs !== null && (
        <div className={css.timingBarWrap}>
          <div className={css.timingBar} aria-hidden="true">
            <i className={css.timingSegmentTtft} style={{ width: `${ttftPercent}%` }} />
            <i className={css.timingSegmentGeneration} style={{ width: `${100 - ttftPercent}%` }} />
          </div>
          <div className={css.timingLegend}>
            <span>
              <i className={css.timingLegendTtft} aria-hidden="true" />
              {t('timing.ttft')} {formatDurationMs(ttftMs, t)}
            </span>
            <span>
              <i className={css.timingLegendGeneration} aria-hidden="true" />
              {t('timing.generation')} {formatDurationMs(generationMs, t)}
            </span>
          </div>
        </div>
      )}
...
export function TrajectoryTable({
  ...
  // Details-panel mount animation seat (effect lives after the selection
  // derivations it reads).
  ...
  const requestInfoByGroup = useMemo(() => {
    const byGroup = new Map<string, { number: number; status?: 'running' | 'error' | 'complete' }>()
    for (const request of sessionRequestNumbers ?? []) {
      byGroup.set(requestKey(request.turn, request.group), {
        number: request.number,
        ...(request.status === undefined ? {} : { status: request.status }),
      })
    }
    for (const [key, number] of requestNumbers) {
      if (byGroup.has(key)) continue
      const recordsOfGroup = allRecords.filter(record => requestKey(record.turn, record.group) === key)
      const status = recordsOfGroup.some(record => record.cell.isError === true)
        ? 'error' as const
        : recordsOfGroup.some(record => stateOf(record) === 'running')
          ? 'running' as const
          : undefined
      byGroup.set(key, { number, ...(status === undefined ? {} : { status }) })
    }
    return byGroup
  }, [allRecords, requestNumbers, sessionRequestNumbers])
  const activeTurnEarly = selectedRequest === null
    ? selected?.turn
    : sessionRequestNumbers?.find(request =>
      requestIdentity(request) === selectedRequest.identity)?.turn
  ...
  // v4 structure rows: turn heads and step annotations decorate the ledger
  // before virtual projection (see trajectory-structure-rows.ts).
  const ledgerRows = useMemo(
    () => decorateLedgerRows(
      records,
      turns,
      {
        searchMode: searchMatchIndexes !== null,
        collapsedTurns,
        requests: requestInfoByGroup,
        activeTurn: activeTurnEarly,
      },
    ),
    [
      activeTurnEarly,
  ...
  const renderedRecords = virtualizationEnabled
    ...
    : ledgerRows.map((record, position) => ({
  ...
  // Details-panel mount animation: slide in from the panel edge whenever a
  // selection opens it. Owned gsap context, motion-gated.
  const detailsOpen = selectedRequestInfo !== undefined
    || promptSelected
    || (selected !== undefined && selectedState !== undefined)
  useEffect(() => {
    if (!detailsOpen || !motionAllowed()) return undefined
    const ctx = gsap.context(() => {
      gsap.from(detailsRef.current, { opacity: 0, x: 24, duration: 0.28, ease: 'power2.out' })
    })
    return () => { ctx.revert() }
  }, [detailsOpen])
  ...
  useEffect(() => {
    ...
    if (id === null) return
    const position = ledgerRows.findIndex(record =>
    ...
    pendingScrollRecordId.current = null
    followsTableTail.current = false
    const recordIndex = ledgerRows[position]?.cell.index
  ...
  }, [ledgerRows, rowVirtualizer, virtualIndexByRecordId, virtualizationEnabled])
  useEffect(() => {
    if (timelineFocusIndexes === null || timelineFocusIndexes.size === 0) return
    const focusedPositions = ledgerRows.flatMap((record, position) =>
    ...
    const focusedVirtualIndexes = [...new Set(focusedPositions.flatMap((position) => {
      const record = ledgerRows[position]
      if (record === undefined) return []
    ...
    <div ref={rootRef} className={css.split} style={splitStyle}>
      ...
      >
        ...
        <table
          ...
          aria-rowcount={ledgerRows.length + historyRowOffset}
        >
          <colgroup>
          ...
          <tbody>
            ...
            {renderedRecords.map(({ record, position }) => {
              if (record.head !== undefined) {
                ...
                return (
                  <tr
                    ...
                    className={css.turnHead}
                    data-row="head"
                    data-turn={head.turn}
                    data-head-active={head.active || undefined}
                    aria-rowindex={position + 1 + historyRowOffset}
                    aria-label={`${t('row.turnAria')} ${head.turn}${
                      head.title === '' ? '' : `, ${head.title}`
                    }`}
                    aria-expanded={head.collapsible ? !head.collapsed : undefined}
                    tabIndex={head.collapsible ? 0 : -1}
                    onClick={head.collapsible ? () => { onToggleTurn(head.turn) } : undefined}
                    onKeyDown={(event) => {
                      if (!head.collapsible || (event.key !== 'Enter' && event.key !== ' ')) return
                      event.preventDefault()
                      onToggleTurn(head.turn)
                    }}
                  >
                    <td colSpan={2}>
                      <span className={css.headMain}>
                        <svg
                          className={css.headChev}
                          viewBox="0 0 16 16"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="1.5"
                          ...
                          strokeLinejoin="round"
                          aria-hidden="true"
                          data-collapsed={head.collapsed || undefined}
                        >
                          <path d="m4 6 4 4 4-4" />
                        </svg>
                        <span
                          className={head.active
                            ? `${css.headIndex} ${css.headIndexActive}`
                            : css.headIndex}
                          aria-hidden="true"
                        >
                          T{head.turn}
                        </span>
                        <span className={css.headTitle}>{head.title === '' ? '—' : head.title}</span>
                        <span className={css.headMetrics}>
                          {head.steps} {head.steps === 1 ? 'step' : 'steps'} · {head.calls} {head.calls === 1 ? 'call' : 'calls'}
                          {head.seconds !== null && ` · ${formatHeadDuration(head.seconds)}`}
                          {head.tokens !== null && ` · ${formatTokenSummary(head.tokens)} tok`}
                        </span>
                      </span>
                    </td>
                  </tr>
              ...
              if (record.step !== undefined) {
                ...
                const requestInfo = step.requestNumber === undefined
                  ? undefined
                  : sessionRequestNumbers?.find(candidate => candidate.number === step.requestNumber)
                const stepLabel = step.requestNumber === undefined
                  ? undefined
                  : `${t('row.stepAria')} #${step.requestNumber}${
                    requestInfo?.purpose === 'compaction' ? ` · ${t('request.compactionPurpose')}` : ''
                  }`
                const stepSelected = requestInfo !== undefined
                ...
                return (
                  <tr
                    ...
                    data-row="step"
                    aria-rowindex={position + 1 + historyRowOffset}
                    aria-label={step.requestNumber === undefined
                      ? step.label
                      : `${t('row.stepAria')} #${step.requestNumber}, ${step.label}`}
                  >
                    <td colSpan={2}>
                      <span className={css.stepMain}>
                        {step.requestNumber !== undefined && (
                          <button
                            type="button"
                            className={stepSelected
                              ? `${css.stepReq} ${css.stepReqOn}`
                              : css.stepReq}
                            aria-label={stepLabel}
                            aria-pressed={stepSelected}
                            data-request-status={step.requestStatus}
                            onClick={(event) => {
                        ...
                        <span className={css.stepName}>{step.label}</span>
                        <i className={css.stepRule} aria-hidden="true" />
                        {step.seconds !== null && (
                          <span className={css.stepSeconds}>{formatHeadDuration(step.seconds)}</span>
                        )}
                      </span>
                    </td>
                  </tr>
              ...
              return (
                ...
                >
                  {({ displayText, listDisplayText, resultText, toolCallOnly, toolCallText }) => {
                    ...
                    const request = isCollapsedSummary || isRequestOnly
                      ? undefined
                    ...
                    const metrics = isCollapsedSummary
                      ? undefined
                      : trajectoryRowMetrics(record.cell, state, t('row.running'))
                    ...
                    return (
                      <tr
                        ...
                        data-request-selected={requestSelected || undefined}
                      ...
                      >
                        <td className={css.event}>
                          ...
                          >
                            <i className={css.kindTick} aria-hidden="true" />
                            <Tooltip
                              ...
                              <>
                                ...
                                >
                                  ...
                                  {resultText !== undefined && (
                                    <span className={record.cell.isError
                                      ? `${css.inlineResult} ${css.error}`
                                      : css.inlineResult}
                                    >
                                ...
                                {state === 'error'
                                  ? (
                                    <span className={`${css.du} ${css.duFail}`} aria-hidden="true">
                                      {t('row.failed')}
                                    </span>
                                  )
                                  : metrics !== undefined && (
                                    <span
                                      aria-hidden="true"
                                      className={`${css.du}${
                                        metrics.tone === 'slow' ? ` ${css.duSlow}` : ''
                                      }${
                                        metrics.tone === 'xslow' ? ` ${css.duXslow}` : ''
                                      }${metrics.running ? ` ${css.duRun}` : ''}`}
                                    >
                                      {metrics.text}
                                    </span>
                                  )}
