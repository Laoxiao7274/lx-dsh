...
import { Button, IconPlusOutline16, Modal, motionAllowed } from '@deepseek-ai/dsh-client-ui-primitives'
...
function Loaded({ injected, renderSlot }: { injected: ModelsSectionFace; renderSlot: ModelsRenderSlot }): ReactNode {
  ...
  // The provider list rises in with a short stagger on mount — the page's one
  // orchestrated entrance; everything after it reacts to the user. Owned gsap
  // context, motion-gated like every decorative tween here.
  const rowsMounted = useRef(false)
  useEffect(() => {
    if (rowsMounted.current || state.status !== 'ready') return
    rowsMounted.current = true
    if (!motionAllowed()) return
    const ctx = gsap.context(() => {
      gsap.from(rowsRef.current?.children ?? [], { opacity: 0, y: -8, duration: 0.28, stagger: 0.05, ease: 'power2.out' })
    }, rowsRef.current ?? undefined)
    return () => { ctx.revert() }
  }, [state.status])

  // The saved confirmation floats in each time an apply lands.
  useEffect(() => {
    if (savedTarget === undefined || !motionAllowed()) return
    const ctx = gsap.context(() => {
      gsap.from(noticeRef.current, { opacity: 0, y: -4, duration: 0.22, ease: 'power2.out' })
    })
    return () => { ctx.revert() }
  }, [savedTarget])
  ...
  return (
    <div className={styles['section']}>
      ...
      {savedIdentity === undefined
        ? null
        : (
          <p ref={noticeRef} className={styles['savedNotice']} role="status" aria-live="polite">
