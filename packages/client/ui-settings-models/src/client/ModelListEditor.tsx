...
import { Button, Modal, motionAllowed } from '@deepseek-ai/dsh-client-ui-primitives'
...
/** Whether a row explicitly declares image input (the vision toggle's mirror). */
function visionOn(model: ModelDraft): boolean {
...
export function ModelListEditor(props: ModelListEditorProps): ReactNode {
  ...
  const prevCount = useRef(models.length)
  const panelRefs = useRef(new Map<number, HTMLDivElement>())
  const prevExpanded = useRef<ReadonlySet<number>>(expanded)

  // Same motion discipline as the DeepSeek catalog editor: staggered first
  // paint, single-row slide on append, disclosure slide, all in owned gsap
  // contexts so superseded or unmounting tweens revert cleanly.
  useEffect(() => {
    const list = listRef.current
    const grew = models.length > prevCount.current
    const firstPaint = prevCount.current === 0
    prevCount.current = models.length
    if (list === null || !motionAllowed() || models.length === 0) return undefined
    const ctx = gsap.context(() => {
      if (firstPaint) {
        gsap.from(Array.from(list.children), { opacity: 0, y: -6, duration: 0.24, stagger: 0.05, ease: 'power2.out' })
      } else if (grew) {
        const added = list.children[models.length - 1]
  ...
  /**
   * Toggle the row's declared request modalities. On adds `image` beside the
   * existing entries; off removes it — pi-ai semantics differ from the
   * DeepSeek catalog here: an explicit list REPLACES the installed catalog
   * entry's declaration, so off writes a deterministic text-only list rather
   * than clearing the field, which would re-arm whatever the catalog records.
   */
  const toggleVision = (index: number): void => {
    const model = models[index]
    if (model === undefined) return
    const current = Array.isArray(model['input'])
      ? (model['input'] as unknown[]).filter((modality): modality is string => typeof modality === 'string')
      : []
    const next = current.includes('image')
      ? current.filter(modality => modality !== 'image')
      : Array.from(new Set(['text', ...current, 'image']))
    const normalized = next.length === 0 ? ['text'] : next
    patch(index, { input: normalized })
  }
        ...
        {models.map((model, index) => (
          <div key={index} className={styles['modelEntry']}>
            <div className={styles['modelRow']}>
              ...
              <ModelVisionChip
                on={visionOn(model)}
                label={`${t('modelVision')} ${String(index + 1)}`}
                disabled={disabled}
