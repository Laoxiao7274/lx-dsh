 ...
 * beneath the key field the card is two flat zones separated by a hairline —
 * the per-family connection extras (`baseURL` for both families, and the
 * display name and wire protocol of a pi-ai route the adapter does not ship)
 * and the model catalog (DeepSeek's id/name/context-window list or the pi-ai
 * list with its endpoint interrogation).
...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  ...
  // The card mounts fresh each time it opens; a short rise-and-fade reads as
  // the row handing over to its editor. Owned gsap context, motion-gated.
  useEffect(() => {
    if (!motionAllowed()) return undefined
    const ctx = gsap.context(() => {
      gsap.from(cardRef.current, { opacity: 0, y: -8, duration: 0.25, ease: 'power2.out' })
    })
    return () => { ctx.revert() }
  }, [])
    ...
    return (
      <>
        ...
        {props.credentialOnly === true ? null : <>
          {/* Connection fields and the model catalog are two flat zones of one
              card, separated by a hairline — the catalog is the page's main
              content, not an advanced drawer. */}
  ...
  return (
    <div ref={cardRef} className={props.credentialOnly === true ? styles['addBlock'] : styles['editor']}>
