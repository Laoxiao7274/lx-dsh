...
describe('ReasoningRow', () => {
  ...
  it('shows the completed thinking duration under the expanded body', () => {
    const view = render(
      <AssistantMarkdown
        t={t}
        blocks={[{ kind: 'reasoning', text: 'Inspect the session' }]}
    ...
    fireEvent.click(view.getByText('思考'))
    expect(view.getByText('思考了 5秒')).toBeTruthy()
  })

  it('omits the duration line when the step timing was never recorded', () => {
    const view = render(
      <AssistantMarkdown
        t={t}
        blocks={[{ kind: 'reasoning', text: 'Inspect the session' }]}
    ...
    fireEvent.click(view.getByText('思考'))
    expect(view.queryByText(/思考了/)).toBeNull()
  })
})
