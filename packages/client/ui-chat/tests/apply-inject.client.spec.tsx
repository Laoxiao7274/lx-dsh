...
describe('Chat inject API', () => {
  ...
  it('restores the retracted prompt text into the composer draft after rewind', async () => {
    const b = await bench()
    await b.runtime.sessions.replaceEvents(ROOT, [
      { type: 'event', event: { type: 'turn/start', seq: SessionSeq(0), time: 1, data: { turn: 1 } } },
      {
        type: 'event',
        event: {
          type: 'user/message', seq: SessionSeq(1), time: 2,
          data: createUserMessage({
            content: [{ type: 'text', text: 'retract me' }, { type: 'text', text: 'with two blocks' }],
            source: { kind: 'user' },
          }),
        },
      },
    ] as never)
    const actx = b.runtime.sessions.scope(ROOT)
    if (actx === undefined) throw new Error('test bench: session scope missing')
    const conversation = actx.get('conversation')
    if (conversation === undefined) throw new Error('test bench: conversation service missing')
    const setDraft = vi.fn()
    vi.spyOn(conversation.input, 'for').mockReturnValue({ setDraft } as never)

    const { injected } = b.chatViewApi(ROOT)
    injected.rewindAt(1)
    await vi.waitFor(() => {
      expect(b.runtime.sessions.calls).toContainEqual({ method: 'rewind', args: [{ sessionId: ROOT, atSeq: 1 }] })
    })
    await vi.waitFor(() => {
      expect(setDraft).toHaveBeenCalledWith('retract me\nwith two blocks')
    })
    await b.runtime.dispose()
  })

  it('leaves the draft untouched when the retracted message carries no text', async () => {
    const b = await bench()
    await b.runtime.sessions.replaceEvents(ROOT, [
      { type: 'event', event: { type: 'turn/start', seq: SessionSeq(0), time: 1, data: { turn: 1 } } },
      {
    ...
    injected.rewindAt(2)
    await vi.waitFor(() => {
      expect(b.runtime.sessions.calls).toContainEqual({ method: 'rewind', args: [{ sessionId: ROOT, atSeq: 2 }] })
    })
