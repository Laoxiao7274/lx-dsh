...
/**
 * Extract the plain-text body of the user message at one seq, for restoring a
 * retracted prompt into the composer draft on rewind.
 * @param entries - the session's current event window.
 * @param seq - the retracted `user/message` event's seq.
 * @returns the concatenated text blocks, or undefined when the message is
 *   absent or carries no text.
 */
function textOfUserMessageAt(entries: readonly SessionEventLikeEntry[], seq: number): string | undefined {
  for (const entry of entries) {
    if (entry.type !== 'event') continue
    const event = entry.event
    if (event.seq !== seq || event.type !== 'user/message') continue
    ...
    const disposeView = ctx.slots.register({
      ...
      inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
        ...
        return {
          ...
          rewindAt: (seq) => {
            // The host truncates the durable log at the boundary's turn and the
            // manager reloads the window, so the conversation returns to before
            // the message. The retracted prompt's text returns to the composer
            // draft, captured before the truncation drops it from the window.
            const retracted = textOfUserMessageAt(binding.eventSource.getSnapshot().entries, seq)
            void ctx.sessions.rewind({ sessionId, atSeq: seq })
              .then(() => {
                if (retracted === undefined || retracted === '') return
                const actx = binding.ctx
                const conversation = actx.get('conversation')
                conversation?.input.for(actx).setDraft(retracted)
              })
              .catch(() => {
                // Rewind failure keeps the source view unchanged.
              })
          },
        }
      },
