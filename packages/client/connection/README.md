...
Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier. Each process mints a random launch token. `dsh-web-app` prints and opens the ordinary root URL with `?token=...`; `frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`, which accepts that token only on `GET /`, writes an authority-bound signed cookie, and redirects to clean `/`. A missing, expired, malformed, or wrong-authority cookie returns 401 before RPC dispatch. Static assets remain public. Beyond the root exchange, non-browser API clients authenticate with the same launch token: HTTP requests carry `Authorization: Bearer <token>`, and WebSocket upgrades (which browsers cannot header-authorize) accept a `?token=` query parameter. A presented token that does not constant-time-match the launch token falls through to the cookie check and 401.
...
API Gateway Client registers the internal `$events` logical stream as the sole generation source, independently of whether any `$on` listener exists. The Host attaches all incremental listeners in the API Remotes source factory, then sends one `{ type: 'ready', clientId, host: { home, protocolVersion } }` item before events; the Client rejects a `protocolVersion` that differs from its own wire version with an explicit version error instead of failing later inside payload validation. `ConnectionController` publishes that generation and calls `onConnected` only after the ready item arrives, so baseline acquisition cannot race ahead of incremental observation.
