...
describe('connection node half', () => {
  ...
  it('shares its configured trust and authentication policy with sibling routes', async () => {
    ...
    // Non-browser credentials: the launch token as an Authorization Bearer
    // header (HTTP clients) and as the token query parameter (WebSocket
    // upgrades, which browsers cannot header-authorize).
    const launchToken = new URL(connection.authenticatedUrl('http://harness.example')).searchParams.get('token') ?? ''
    ...
    expect(connection.requestRejection(fakeRequest(
      { host: 'harness.example' },
      `${API_PATH}/session.list?token=${encodeURIComponent(launchToken)}`,
    ))).toBeUndefined()
    expect(connection.requestRejection(fakeRequest({
      host: 'harness.example',
      authorization: 'Bearer not-the-launch-token',
    }))).toBe(401)
    expect(connection.requestRejection(fakeRequest(
      { host: 'harness.example' },
      `${API_PATH}/session.list?token=not-the-launch-token`,
    ))).toBe(401)
    await dispose()
  })
