...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  requestRejection(request: ConnectionTrustRequest): ConnectionRequestRejection {
    ...
    if (this.queryLaunchToken(request) !== undefined) return undefined
    return 401
  }

  /**
   * Read the `token` query parameter for non-browser clients. A browser
   * WebSocket upgrade cannot set an Authorization header, so the launch
   * token rides the URL; ordinary HTTP clients prefer the header.
   * @param request - incoming request whose URL may carry ?token=.
   * @returns the verified-present marker, or undefined when absent/invalid.
   */
  private queryLaunchToken(request: ConnectionTrustRequest): true | undefined {
    /* v8 ignore next -- node:http always sets url on server requests. */
    const url = new URL(request.url ?? '/', 'http://dsh.invalid')
    const values = url.searchParams.getAll('token')
    return values.length === 1 && this.browserAuth.verifyLaunchToken(values[0] ?? '') ? true : undefined
  }
