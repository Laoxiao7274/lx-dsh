...
export function Menu({ open, anchor, items, selectedId, selectedIds, onSelect, onClose, align = 'start', side = 'bottom', portal = false, closeOnPointerLeave = false, dense = false, compact = false, getAnchorRect, footer, className }: {
  ...
  // Entrance: fade/rise from the anchor edge. Runs after the portal placement
  // pass (declared above), so the tween's from-state is already at the final
  // position — no visible jump from a mid-measure frame.
  useLayoutEffect(() => {
    if (!open || !motionAllowed()) return
    const el = listRef.current
    if (el === null) return
    const rise = side === 'top' ? 4 : -4
