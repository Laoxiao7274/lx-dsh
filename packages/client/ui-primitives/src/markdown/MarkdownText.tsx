...
import { useStreamReveal } from './stream-reveal.ts'
import {
...
export const MarkdownText = memo(function MarkdownText({ text, streaming = false, labels, fileMentions }: {
  ...
  // Keep the streaming arm mounted until the paced reveal caught up, so the
  // settle swap never truncates text that is still flowing in.
  const streamingArm = streaming || shown !== text
  const children = useMemo(() => {
    ...
    return streamRef.current.render(shown)
  }, [shown, streamingArm, text, labels, fileMentions])
  return (
    <div className={css.markdown} data-md-streaming={streamingArm ? 'true' : undefined}>
      {children}
    </div>
  )
})
