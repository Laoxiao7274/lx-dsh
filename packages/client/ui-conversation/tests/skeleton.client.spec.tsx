...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, bare header keeps utilities and tabs, picker switches the workspace', () => {
    const b = mount(
    ...
    const header = b.view.container.querySelector('header')
    expect(host).not.toBeNull()
    // The header stays visible in the blank phase: the desktop shell's window
    // chrome and the export action live in its utilities strip, while the
    // title cluster (a provisional session has no meaningful crumb) is dropped.
    expect(header).not.toBeNull()
    expect(header?.getAttribute('aria-hidden')).toBeNull()
    expect(header?.querySelector('[class*="titleCluster"]')).toBeNull()
    expect(header?.querySelector('[class*="headerUtilities"]')).not.toBeNull()
