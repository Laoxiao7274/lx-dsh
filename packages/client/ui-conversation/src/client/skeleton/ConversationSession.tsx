 ...
 * A pristine blank draft keeps the header on screen: the title cluster (crumbs
 * and actions) is omitted — a provisional session has no meaningful title to
 * show — while the utilities strip and the view tabs stay, so the desktop
 * shell's window chrome and the export action never disappear.
 * @param props - Strict Session store, view ledger, navigation, render, and locale shares.
 * @returns the blank-session header (utilities and tabs only) or the full title and tabs.
 */
export function ConversationSessionHeader({
  ...
  const bareChrome = session.blank && conversationPhase(session, conversation) === 'blank'

  return (
    <header className={clsx(css.header, bareChrome && css.headerBlank)}>
