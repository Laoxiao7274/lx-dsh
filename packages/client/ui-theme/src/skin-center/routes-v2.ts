...
import {
  canServeSkinHooks, findSkin, loadSkinCatalog, repairSkin, resolveInsideSkin,
  shippedSkinIds, uninstallUserSkin, verifyAndRepairAllSkins, userSkinsDir,
} from './skin-repo.ts'
...
export function makeSkinCenterV2Routes(deps: RoutesV2Deps = {}): WebRoute[] {
  ...
  const verifyHandler: WebRoute['handler'] = async (req, res) => {
    ...
    const result = await verifyAndRepairAllSkins(loadCatalog, {
      ...(deps.userDir === undefined ? {} : { userDir: deps.userDir }),
      ...(deps.fetchImpl === undefined ? {} : { fetchImpl: deps.fetchImpl }),
      ...(deps.localSourceDir === undefined ? {} : { localSourceDir: deps.localSourceDir }),
      autoRepair,
    })
  ...
  const skinsHandler: WebRoute['handler'] = async (req, res) => {
    ...
    if (sub === 'uninstall') {
      ...
      const userDir = deps.userDir ?? (entry.dir ? dirname(entry.dir) : userSkinsDir())
  ...
  const activePostHandler: WebRoute['handler'] = async (req, res) => {
    ...
    if (hasBackground) {
      const background = sanitizeSkinBackground(record.background)
      if (background === null) {
