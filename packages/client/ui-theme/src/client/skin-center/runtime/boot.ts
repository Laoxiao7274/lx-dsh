...
export function bootSkinRuntime(options: BootOptions = {}): SkinRuntimeStore {
  ...
  const controller = createSkinController({
    ...
    apiBase,
    fetchImpl,
    ...(options.suppressBackgroundMedia === undefined ? {} : { suppressBackgroundMedia: options.suppressBackgroundMedia }),
  ...
  if (typeof doc.defaultView !== 'undefined' && doc.defaultView !== null) {
    ...
    const onSkinApplied = (e: Event) => {
      ...
      if (detail && typeof detail.id === 'string') {
        const appliedId = detail.id
        void refreshCatalog().then(async () => {
          const entry = store.find(appliedId)
          if (entry !== null) {
            await controller.switchTo(appliedId, entry as ControllerSkinEntry)
          }
        })
      }
    }
