...
describe('JsonlSessionPersistence: default Zstandard encoding', () => {
  ...
  it('tolerates a foreign end-seed marker colliding with the live writer\'s next row', async () => {
    // Two processes shared this artifact: a cold-open appended the marker at
    // the live cursor, then the live writer re-covered its seq with the real
    // next row. Reads merge losslessly without rewriting the artifact.
    const root = await freshRoot()
    const ctx = await mount(root)
    const warn = vi.spyOn(ctx.logger, 'warn').mockImplementation(() => undefined)
    const header = meta('marker-collision', '/proj')
    await writeLog(ctx.sessionPersistence, header, oneTurnLog()) // seqs 0..5
    const path = logPath(root, header.cwd, header.id, 'zstd')
    const committed = await readFile(path)
    const marker = JSON.stringify({ type: 'session/end-seed', seq: SessionSeq(6), time: 7, data: {} }) + '\n'
    const restart: SessionEvent[] = [
      { type: 'turn/start', seq: SessionSeq(6), time: 8, data: { turn: 2 } },
      { type: 'turn/end', seq: SessionSeq(7), time: 9, data: { turn: 2, reason: { kind: 'completed' } } },
    ]
    await appendFile(path, await compressZstdFrame(marker))
    await appendFile(path, await compressZstdFrame(restart.map(e => JSON.stringify(e)).join('\n') + '\n'))

    const loaded = await readAll(ctx.sessionPersistence, header.id)
    expect(loaded.events).toEqual([...oneTurnLog(), ...restart])
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('stale log row'))
    // The read repaired nothing: the foreign bytes stay until a write lands.
    expect((await readFile(path)).subarray(0, committed.length)).toEqual(committed)

    // The next append truncates the foreign marker's frame and rewrites the
    // surviving rows durably, so a fresh read sees a clean artifact.
    const thirdTurn: SessionEvent[] = [
      { type: 'turn/start', seq: SessionSeq(8), time: 10, data: { turn: 3 } },
      { type: 'turn/end', seq: SessionSeq(9), time: 11, data: { turn: 3, reason: { kind: 'completed' } } },
    ]
    await appendBatch(ctx.sessionPersistence, header.id, thirdTurn)
    const repaired = await readFile(path)
    expect(scanZstdFrames(repaired).tornStart).toBeUndefined()
    expect(scanLog(await decodeCompleteFrames(repaired)).events)
      .toEqual([...oneTurnLog(), ...restart, ...thirdTurn])
  })

  it('bridges a one-event hole with a synthetic turn closer when the next row opens a new turn', async () => {
    // A truncate-while-live race removed exactly the interrupted turn's
    // turn/end: reads synthesize it deterministically, so history stays whole.
    const root = await freshRoot()
    const ctx = await mount(root)
    const header = meta('hole-bridge', '/proj')
    const openTurn: SessionEvent[] = [
      { type: 'agent/inbox/spliced', seq: SessionSeq(0), time: 1, data: { target: 'next-turn', start: 0, inserted: [] } },
      { type: 'turn/start', seq: SessionSeq(1), time: 2, data: { turn: 1 } },
      { type: 'agent/inbox/spliced', seq: SessionSeq(2), time: 3, data: { target: 'next-turn', start: 0, removedCount: 1, inserted: [] } },
      // hole: the interrupted turn/end at seq 3 never landed on disk
    ]
    const nextTurn: SessionEvent[] = [
      { type: 'step/start', seq: SessionSeq(4), time: 5, data: { turn: 2, step: 1 } },
      { type: 'step/end', seq: SessionSeq(5), time: 6, data: { turn: 2, step: 1 } },
      { type: 'turn/end', seq: SessionSeq(6), time: 7, data: { turn: 2, reason: { kind: 'completed' } } },
    ]
    const handle = await ctx.sessionPersistence.create(header)
    try {
      await handle.append(openTurn)
    } finally {
      await handle.close()
    }
    const path = logPath(root, header.cwd, header.id, 'zstd')
    await appendFile(path, await compressZstdFrame(nextTurn.map(e => JSON.stringify(e)).join('\n') + '\n'))

    const loaded = await readAll(ctx.sessionPersistence, header.id)
    const closer = loaded.events[3]
    expect(closer).toMatchObject({ type: 'turn/end', seq: SessionSeq(3), data: { turn: 1, reason: { kind: 'interrupted' } } })
    expect(loaded.events.map(event => event.seq)).toEqual([0, 1, 2, 3, 4, 5, 6])
    // The synthetic closer reuses the last real event's timestamp (repair.ts).
    expect(closer?.time).toBe(openTurn[2]!.time)
  })

  it('salvages the prefix before an unbridgeable hole and repairs durably on the next append', async () => {
    // A hole the closers cannot fit (three missing seqs, no open turn) stops
    // the read at its frame: the prefix survives and the write path truncates.
    const root = await freshRoot()
    const ctx = await mount(root)
    const warn = vi.spyOn(ctx.logger, 'warn').mockImplementation(() => undefined)
    const header = meta('hole-cut', '/proj')
    await writeLog(ctx.sessionPersistence, header, oneTurnLog()) // seqs 0..5, balanced
    const path = logPath(root, header.cwd, header.id, 'zstd')
    const committed = await readFile(path)
    const jumpingBatch: SessionEvent[] = [
      { type: 'turn/start', seq: SessionSeq(9), time: 20, data: { turn: 2 } },
      { type: 'turn/end', seq: SessionSeq(10), time: 21, data: { turn: 2, reason: { kind: 'completed' } } },
    ]
    await appendFile(path, await compressZstdFrame(jumpingBatch.map(e => JSON.stringify(e)).join('\n') + '\n'))

    const loaded = await readAll(ctx.sessionPersistence, header.id)
    expect(loaded.events).toEqual(oneTurnLog())

    // The first append truncates the hole's frame and writes from the salvaged
    // cursor, so the artifact heals to a clean contiguous log.
    const resumed: SessionEvent[] = [
      { type: 'turn/start', seq: SessionSeq(6), time: 30, data: { turn: 2 } },
      { type: 'turn/end', seq: SessionSeq(7), time: 31, data: { turn: 2, reason: { kind: 'completed' } } },
    ]
    await appendBatch(ctx.sessionPersistence, header.id, resumed)
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('recovered from a torn tail'))
    const repaired = await readFile(path)
    expect(repaired.subarray(0, committed.length)).toEqual(committed)
    expect(scanZstdFrames(repaired).tornStart).toBeUndefined()
    expect(scanLog(await decodeCompleteFrames(repaired)).events).toEqual([...oneTurnLog(), ...resumed])
  })
