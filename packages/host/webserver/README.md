...
Set `corsOrigins` to a list of exact browser origins (for example `https://app.example.com`) to allow those origins to call this server cross-origin. Matching requests carry `Access-Control-Allow-Origin` (the exact origin, never `*`), `Access-Control-Allow-Credentials`, and `Vary: Origin`; `OPTIONS` preflights answer 204 with the methods and headers the browser API needs; WebSocket upgrades whose `Origin` header is not allowlisted are refused. Empty (the default) emits no CORS headers — same-origin clients are unaffected. The browser, not this server, is the enforcement point: requests with other or absent origins pass through untouched, so non-browser clients never need to change.
