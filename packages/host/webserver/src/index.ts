...
export interface Config {
  ...
  port: number
  /**
   * Exact browser origins allowed to call this server cross-origin. A
   * matching request carries `Access-Control-Allow-Origin` (the exact
   * origin, never `*`), `Access-Control-Allow-Credentials`, and `Vary:
   * Origin`; OPTIONS preflights answer 204; and WebSocket upgrades whose
   * Origin header is not allowlisted are rejected. Empty (the normalized
   * default) keeps the same-origin-only behavior — no CORS headers are
   * emitted.
   */
  corsOrigins: string[]
...
export class WebServer extends Service {
  ...
  private readonly corsAllow: ReadonlySet<string>
  ...
  constructor(ctx: Context, private config: Config) {
    ...
    this.corsAllow = new Set(config.corsOrigins ?? [])
  }
  ...
  async [Service.init](): Promise<void> {
    ...
    this.server.on('upgrade', (req, socket, head) => {
      ...
      // A browser WebSocket upgrade cannot opt out of its page's Origin; when
      // an allowlist is configured, reject every origin it does not name.
      if (this.corsAllow.size > 0) {
        const origin = req.headers.origin
        if (typeof origin !== 'string' || !this.corsAllow.has(origin)) {
  ...
  /**
   * Stamp the CORS response headers for an allowlisted request origin, or
   * answer a preflight outright. Headers are set before route dispatch so
   * every handler outcome — including the fallback and the guard's 400 —
   * carries them. Requests without an Origin header (non-browser clients)
   * and non-allowlisted origins pass through untouched: the browser, not
   * this server, is the CORS enforcement point.
   * @param req - incoming request.
   * @param res - response the headers are stamped on.
   * @returns 'preflight' when the request was answered here, otherwise
   * 'continue'.
   */
  private applyCors(req: IncomingMessage, res: ServerResponse): 'preflight' | 'continue' {
    if (this.corsAllow.size === 0) return 'continue'
    const origin = req.headers.origin
    if (typeof origin !== 'string' || !this.corsAllow.has(origin)) return 'continue'
    ...
    const present = vary === undefined
      ? []
      : Array.isArray(vary)
        ? vary.map(v => String(v))
        : String(vary).split(',').map(v => v.trim()).filter(v => v !== '')
    if (!present.some(v => v.toLowerCase() === 'origin')) {
      res.setHeader('vary', [...present, 'Origin'])
    }
    return 'continue'
  }
