...
class RemoteEventCarrier {
  ...
  private async *iterate(signal: AbortSignal): AsyncGenerator {
    ...
    try {
      yield { type: 'ready', clientId, host: { protocolVersion: 1, home: '/home/fixture' } }
...
describe('Client Remote transport readiness', () => {
  ...
  it('reports Host facts as plain reads and keeps them through Connection withdrawal', async () => {
    ...
    live.snapshot = { id: 1, host: { protocolVersion: 1, home: '/hosts/primary' } }
...
describe('Client Typert API', () => {
  ...
  it('opens the forwarded-event stream on the browser Remote mux', async () => {
    await withFakeWebSocket('https://harness.example', async () => {
      ...
      socket.receive({
        ...
        value: { type: 'ready', clientId: 'browser-client', host: { protocolVersion: 1, home: '/home/browser' } },
      })
      await run.ready
      socket.receive({
        type: 'item',
  ...
  it.each([
    ...
    { type: 'ready', clientId: 'client', host: { protocolVersion: 1, home: 1 } },
    { type: 'ready', clientId: 'client', host: { protocolVersion: 1, home: '/home', extra: true } },
