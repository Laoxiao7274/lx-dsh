...
/**
 * Remote wire protocol version carried by every forwarded-event generation's
 * opening frame. A Client that expects a different major value reports the
 * mismatch instead of failing deep inside payload validation. Bump on any
 * breaking frame change; additive optional fields keep the value.
 */
...
export interface RemoteEventHostInfo {
  ...
  /** Remote wire protocol version the Host speaks; see {@link REMOTE_PROTOCOL_VERSION}. */
