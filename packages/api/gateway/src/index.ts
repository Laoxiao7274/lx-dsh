...
export { REMOTE_PROTOCOL_VERSION } from './stream-protocol.ts'
...
export class TypertGatewayService extends Service implements TypertGateway {
  ...
  private schemaCache: string | undefined
  ...
  constructor(ctx: Context, config: Config) {
    ...
    ctx.inject(['connection'], (connectionCtx) => {
      ...
      // Contract introspection: one GET returns every locally registered
      // Remote invocation with its wire names, parameter sources, and stable
      // type symbols, so a third-party client can be written against the
      // running Host without reading TypeScript sources. Cached until the
      // local registry changes.
      connectionCtx.effect(() => connectionCtx.connection.fetch.register({
        path: '/api/$schema',
        methods: ['GET'],
        fetch: () => {
          // schemaSnapshot() already returns serialized JSON.
          return Promise.resolve(new Response(this.schemaSnapshot(), {
            status: 200,
            headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' },
          }))
        },
      }), 'api-gateway: /api/$schema contract endpoint')
    })
  ...
  registerRemoteEvents(
    ...
    const registration: RegisteredRemoteEventSource = {
      ...
      host: { home: host.home, protocolVersion: host.protocolVersion },
    }
  ...
  /**
   * Render the contract snapshot served by `/api/$schema`: the wire protocol
   * version plus every locally registered invocation, grouped by namespace.
   * Parameters carry their wire names, JSON-or-lookup source, and the stable
   * type symbol each strict codec was generated from. Cached until a local
   * registration changes.
   * @returns the serialized JSON document.
   */
  private schemaSnapshot(): string {
    if (this.schemaCache === undefined) {
      const namespaces = new Map<string, {
        ...
        methods: {
          method: string
          mode: 'unary' | 'stream'
          parameters: { name: string; wire: string; source: 'json' | 'lookup'; typeSymbol: string | undefined }[]
          resultTypeSymbol: string | undefined
        }[]
      }>()
      for (const descriptor of this.ctx.typert.local.list()) {
        const entry = namespaces.get(descriptor.namespace) ?? {
        ...
        entry.methods.push({
          ...
          parameters: descriptor.parameters.map(parameter => ({
            ...
            typeSymbol: parameter.codec.mode === 'strict' ? parameter.codec.typeSymbol : undefined,
          })),
          resultTypeSymbol: descriptor.result.mode === 'strict' ? descriptor.result.typeSymbol : undefined,
        })
        namespaces.set(descriptor.namespace, entry)
      }
      const document = {
        protocolVersion: REMOTE_PROTOCOL_VERSION,
        rpc: { method: 'POST', path: '/api/<namespace>/<method>' },
        streamMuxPath: REMOTE_STREAM_MUX_PATH,
        namespaces: [...namespaces.values()].sort((a, b) => a.namespace.localeCompare(b.namespace)),
      }
      this.schemaCache = JSON.stringify(document, null, 2)
    }
