...
describe('sessions.list cold merge', () => {
  ...
  it('backfills a missing projection-cache row in the background and announces the titled row', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    const meta = header('backfilled-title', 100)
    // A complete titled conversation whose stat-reported artifact is beyond
    // the small-cold probe: the row serves without projections, then the
    // backfill folds one cache row.
    const events: SessionEvent[] = [
      { type: 'turn/start', seq: SessionSeq(0), time: 200, data: { turn: 1 } },
      {
        type: 'user/message', seq: SessionSeq(1), time: 300,
        data: createUserMessage({ content: [{ type: 'text', text: 'first prompt' }], source: { kind: 'user' } }),
        surfaceOp: 'append',
      },
      { type: 'session/title', seq: SessionSeq(2), time: 400, data: { title: 'Backfilled title', messageSeqs: [SessionSeq(1)], source: { kind: 'fallback' } } },
      { type: 'turn/end', seq: SessionSeq(3), time: 500, data: { turn: 1, reason: { kind: 'completed' } } },
    ]
    providePersistence(ctx, {
      list: () => Promise.resolve([meta]),
      stat: () => Promise.resolve(statSnapshot(meta, { sizeBytes: 2048 })),
      inspect: () => Promise.resolve({ meta, events }),
    })
    // The cache row lands through coldSnapshot's write-back; a later list read
    // serves the folded block (coldSnapshot is the row, so mirror it forward).
    let rowWritten = false
    const coldSnapshots: SessionHeader[] = []
    ctx.provide('sessionProjectionCache', {
      cachedSnapshot: () => (rowWritten
        ? {
          asOfSeq: SessionSeq(3),
          values: {
        ...
        : undefined),
      hydratePrepared: (session: Session, log: readonly SessionEvent[]) =>
        ctx.sessionProjections.hydrate(session, {}, log, SessionLogOffset(0)),
      coldSnapshot: (header: SessionHeader) => {
        coldSnapshots.push(header)
        rowWritten = true
        return ctx.sessionProjections.restore(
          {},
          events,
          SessionLogOffset(0),
          meta,
    ...
    const announced: SessionSummary[] = []
    ctx.on('api-session/added', (summary) => { announced.push(summary) })
    const remote = createSessionTestRemote(ctx, {
      defaultModelSelection: () => ({ provider: 'p', model: 'm' }),
      cwd: '/tmp',
    })
    ...
    const response = await remote.list(request({}))
    if (!response.ok) throw new Error('unreachable')
    // The row itself serves without projections (the directory title period).
    expect(response.value.items[0]?.projections).toBeUndefined()

    // The background backfill folds one cache row and announces the titled row.
    await vi.waitFor(() => { expect(coldSnapshots).toEqual([meta]) })
    await vi.waitFor(() => { expect(announced).toHaveLength(1) })
    expect(announced[0]).toMatchObject({
      sessionId: meta.id,
      projections: {
        values: expect.objectContaining({ title: 'Backfilled title' }),
      },
    })
  })
