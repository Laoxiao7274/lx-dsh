...
export interface ISession {
  ...
  /**
   * First open of the event window: pull the tail page and start the live
   * stream (idempotent — in-flight/already-open returns the existing
   * promise). Staging (making the session current) is NOT required: a
   * background consumer (e.g. the quick-answers drawer) may open and follow a
   * session's events without touching the main window's selection.
