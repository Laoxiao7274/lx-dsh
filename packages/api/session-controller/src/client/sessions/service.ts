...
export class ClientSessions implements ISessions {
  ...
  /**
   * Cross-window selection awaiting its target row (the new-session race:
   * another window persisted a selection for a session whose list row has
   * not reached this client yet). Applied on the next list publications.
   */
  private pendingFollow: SessionSelection | undefined
  ...
  constructor(
    ...
    this.selection = createSnapshotStore<SessionSelection>(
      {},
      { persist: { name: SESSION_SELECTION_STORAGE_KEY } })
    ...
    // Cross-window follows retry first, so a row arriving with the list can
    // complete a pending selection before staging runs.
    const disposeStageFollower = this.list.subscribe(() => {
      this.applyPendingFollow()
      this.followCurrent()
    })
    // Cross-window selection following: every same-origin window persists its
    // current selection under one localStorage key, so a storage event from
    // another window re-targets this client through the same selection paths
    // a user click takes (open/openSubagent/clear). All windows of one backend
    // and one browser profile therefore stage the same session and stream it
    // live; other browser profiles keep their own view.
    rootCtx.effect(() => {
      const canListen = typeof globalThis.addEventListener === 'function'
      const onStorage = (event: StorageEvent): void => {
        if (event.key !== SESSION_SELECTION_STORAGE_KEY || event.newValue === null) return
        const next = parsePersistedSelection(event.newValue)
        if (next !== undefined) this.followPersistedSelection(next)
      }
      if (canListen) globalThis.addEventListener('storage', onStorage)
      return () => {
        if (canListen) globalThis.removeEventListener('storage', onStorage)
      }
    }, 'session-controller.client.selection-follow')
  ...
  /**
   * Rewind a session in place: the host truncates the durable event log to
   * the turn boundary before the addressed user message and checkpoints the
   * truncation; the resident conversation window reloads from the truncated
   * log, so the conversation returns to the state before the message.
   * @param opts - session id and the `user/message` event seq to rewind from.
   * @returns the rewind result (transport-shaped error on failure).
   */
  async rewind(opts: { sessionId: SessionId; atSeq: number }): Promise<RemoteResult<{ accepted: true }>> {
    return this.manager.rewind(opts)
  }
  ...
  /**
   * Adopt a selection persisted by another same-origin window. A value equal
   * to this client's selection is an echo of what it already shows; anything
   * else becomes the pending follow and applies as soon as its target is
   * addressable here.
   * @param next - the validated selection parsed from the storage event.
   */
  private followPersistedSelection(next: SessionSelection): void {
    if (sameSelection(next, this.selection.getSnapshot())) return
    this.pendingFollow = next
    if (next.subagentAddress !== undefined) {
      // The addressed child's catalog may not be loaded here yet; pulling it
      // publishes the list, which retries the pending follow.
      void this.manager.refreshSubagents(next.subagentAddress.parentSessionId)
    }
    this.applyPendingFollow()
  }

  /**
   * Try to apply the pending cross-window selection through the same paths a
   * user click takes. A target that is not addressable here yet stays
   * pending — the new-session race never writes a destructive selection —
   * and every later list publication retries here. An ordinary session is
   * addressable once its row is listed; a catalog child only once its
   * parent's catalog here carries a matching healthy entry (child rows are
   * projected solely along the current addressed route, so the catalog, not
   * `byId`, is their addressability source).
   */
  private applyPendingFollow(): void {
    ...
    const address = next.subagentAddress
    if (address === undefined) {
      if (this.list.getSnapshot().byId[id] === undefined) return
      this.pendingFollow = undefined
      this.open(id)
      return
    }
    const entry = this.list.getSnapshot().subagentsByParent[address.parentSessionId]?.entries
      .find(candidate => candidate.kind === 'child' && candidate.id === id)
    if (entry?.kind !== 'child' || entry.mode !== address.mode) return
    this.pendingFollow = undefined
...
/** localStorage key persisting the cross-window current-session selection. */
const SESSION_SELECTION_STORAGE_KEY = 'dsh.sessions.current'

/**
 * Structural equality for persisted selections: same session id and, when
 * one side addresses a catalog child, the same durable parent address.
 * @param left - one validated selection.
 * @param right - the other validated selection.
 * @returns whether both selections name the identical target.
 */
function sameSelection(left: SessionSelection, right: SessionSelection): boolean {
  ...
  const leftAddress = left.subagentAddress
  const rightAddress = right.subagentAddress
  if (leftAddress === undefined || rightAddress === undefined) return leftAddress === rightAddress
  return leftAddress.parentSessionId === rightAddress.parentSessionId
    && leftAddress.childSessionId === rightAddress.childSessionId
    && leftAddress.mode === rightAddress.mode
}

/**
 * Parse and validate one persisted selection written by another window.
 * localStorage is a durable boundary: a value the store never wrote is
 * ignored rather than trusted.
 * @param raw - the storage event's new value.
 * @returns the selection, or undefined when the value is not a persisted
 * selection this client recognizes.
 */
function parsePersistedSelection(raw: string): SessionSelection | undefined {
  let value: unknown
  try {
  ...
  const address = record.subagentAddress
  if (address === undefined) {
    return Object.keys(record).every(key => key === 'sessionId')
  ...
  if (typeof addressRecord.parentSessionId !== 'string'
    || typeof addressRecord.childSessionId !== 'string'
    || (addressRecord.mode !== 'one-shot' && addressRecord.mode !== 'continuable')
    || Object.keys(addressRecord).length !== 3) return undefined
  if (sessionId === undefined) return undefined
  return {
    sessionId: sessionId as SessionId,
    subagentAddress: {
      parentSessionId: addressRecord.parentSessionId as SessionId,
      childSessionId: addressRecord.childSessionId as SessionId,
      mode: addressRecord.mode,
    },
  }
}
