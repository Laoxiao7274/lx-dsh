...
export class SessionCommandController {
  ...
  /**
   * Rewind a live Session in place: the durable log is truncated to the
   * prefix ending at `atSeq`'s turn boundary. A running agent owns the
   * session, so an active turn refuses the rewind.
   * @param request - live Session and the `user/message` boundary to remove from.
   * @returns acknowledgement that the truncation was persisted.
   */
  async rewind(request: SessionRewindRequest): Promise<SessionRewindValue> {
    if (!Number.isInteger(request.atSeq) || request.atSeq < 0) {
    ...
    if (agent !== undefined && agent.status === 'running') {
      throw new RemoteError(
        ...
        `session "${request.sessionId}" is running a turn; wait for it to finish before rewinding`,
        { sessionId: request.sessionId },
      )
    }
    try {
      await this.ctx.sessions.rewind(request.sessionId, request.atSeq)
    } catch (error) {
      throw new RemoteError(
        'session/rewind-unavailable',
        `rewind of session "${request.sessionId}" at event ${String(request.atSeq)} failed: ${String(error)}`,
        { sessionId: request.sessionId },
      )
    }
